    ...
    namespace consensus {
        ...
        public:
            ...
            */
			...
			bool				store_account_entries_for_simulator(const vector<dfdcore::wallet::WalletAccountEntry>& vec_account_entry);
            /**
            ...
            */
            ...
            /**  Fetch DFD input balance in blocks that block_num of them are lower of the given block_num
            *  or equal to
            *
            * @param  block_num  uint32_t
            *
            * @return vector<DfdTrxidBalance>
            */
            vector<DfdTrxidBalance> fetch_dfd_input_balance(const uint32_t &  block_num);

            /**  Insert the DFD transaction balance to db _dfd_input_balance_entry
            *
            * @param  dfd_account  string
            * @param  dfd_balance_entry  DfdTrxidBalance
            *
            * @return void
            */
            void transaction_insert_to_dfd_balance(const string & dfd_account, const DfdTrxidBalance & dfd_balance_entry);

            /** Erase the DFD transaction balance from db _dfd_input_balance_entry
            *
            * @param  dfd_account  string
            * @param  dfd_balance_entry  DfdTrxidBalance
            *
            * @return void
            */
            void transaction_erase_from_dfd_balance(const string & dfd_account, const DfdTrxidBalance & dfd_balance_entry);
            vector<DfdTrxidBalance> fetch_dfd_full_entry(const uint32_t& block_num, const uint32_t& last_scan_block_num);
            //Must handle account before you use this function
            void transaction_insert_to_dfd_full_entry(const string& dfd_accout, const DfdTrxidBalance& dfd_balance_entry);
            //Must handle account before you use this function
            void transaction_erase_from_dfd_full_entry(const string& dfd_accout, const DfdTrxidBalance& dfd_balance_entry);
...
FC_REFLECT(dfdcore::consensus::BlockForkData, (next_blocks)(is_linked)(is_valid)(invalid_reason)(is_included)(is_known))
FC_REFLECT(dfdcore::consensus::ForkEntry, (block_id)(signing_delegate)(transaction_count)(latency)(size)(timestamp)(is_valid)(invalid_reason)(is_current_fork))
