    ...
    namespace consensus {
        ...
        namespace detail
        {
            ...
            public:
                ...
                * Get database_version from property database and check whether it equals to DFD_BLOCKCHAIN_DATABASE_VERSION
                ...
                */
                ...
                dfdcore::db::LevelMap<TransactionIdType, SignedTransaction>                 _pending_transaction_db;
                map<fee_index, TransactionEvaluationStatePtr>                            _pending_fee_index;
                ShareType                                                                  _relay_fee = DFD_BLOCKCHAIN_DEFAULT_RELAY_FEE;

                /* Block processing */
                uint32_t /* Only used to skip undo states when possible during replay */    _min_undo_block = 0;

                fc::mutex                                                                   _push_block_mutex;
                ShareType															_block_per_account_reword_amount = DFD_MAX_DELEGATE_PAY_PER_BLOCK;
                dfdcore::db::LevelMap<BlockIdType, FullBlock>                               _block_id_to_full_block;
                dfdcore::db::fast_level_map<BlockIdType, PendingChainState>                 _block_id_to_undo_state;

                dfdcore::db::LevelMap<uint32_t, vector<BlockIdType>>                         _fork_number_db; // All siblings
                dfdcore::db::LevelMap<BlockIdType, BlockForkData>                          _fork_db;

                dfdcore::db::LevelMap<BlockIdType, int32_t>                                  _revalidatable_future_blocks_db; //int32_t is unused, this is a set

                dfdcore::db::LevelMap<uint32_t, BlockIdType>                                 _block_num_to_id_db; // Current chain

                dfdcore::db::LevelMap<BlockIdType, BlockEntry>                             _block_id_to_block_entry_db; // Statistics

                /* Current primary state */
                BlockIdType                                                               _head_block_id;
                SignedBlockHeader                                                         _head_block_header;

                dfdcore::db::fast_level_map<uint8_t, PropertyEntry>                           _property_id_to_entry;

                dfdcore::db::fast_level_map<AccountIdType, AccountEntry>                    _account_id_to_entry;
                dfdcore::db::fast_level_map<string, AccountIdType>                            _account_name_to_id;
                dfdcore::db::fast_level_map<Address, AccountIdType>                           _account_address_to_id;
                set<VoteDel>                                                               _delegate_votes;

                dfdcore::db::fast_level_map<AssetIdType, AssetEntry>                        _asset_id_to_entry;
                dfdcore::db::fast_level_map<string, AssetIdType>                              _asset_symbol_to_id;

                dfdcore::db::fast_level_map<SlateIdType, SlateEntry>                        _slate_id_to_entry;

                dfdcore::db::fast_level_map<BalanceIdType, BalanceEntry>                    _balance_id_to_entry;
                dfdcore::db::fast_level_map<string, set<DfdTrxidBalance>>							_dfd_input_balance_entry;
                dfdcore::db::fast_level_map<string, DfdBalanceEntry>						_dfd_full_entry;
                dfdcore::db::LevelMap<TransactionIdType, TransactionEntry>                 _transaction_id_to_entry;
                set<UniqueTransactionKey>                                                 _unique_transactions;
                dfdcore::db::LevelMap<Address, unordered_set<TransactionIdType>>             _address_to_transaction_ids;




                dfdcore::db::LevelMap<SlotIndex, SlotEntry>                                 _slot_index_to_entry;
                dfdcore::db::LevelMap<time_point_sec, AccountIdType>                         _slot_timestamp_to_delegate;

                dfdcore::db::LevelMap<int, data_version>                      _block_extend_status;
                // TODO: Just store whitelist in asset_entry
                //dfdcore::db::level_map<pair<asset_id_type,address>, object_id_type>             _auth_db;

                map<OperationTypeEnum, std::deque<Operation>>                             _recent_operations;

                // contract related db
				dfdcore::db::fast_level_map<std::string, ContractTemplateEntry>				_bytecode_hash_permitted;
                dfdcore::db::fast_level_map<ContractIdType, ContractEntry>                  _contract_id_to_entry;
                dfdcore::db::fast_level_map<ContractIdType, ContractStorageEntry>               _contract_id_to_storage;
                dfdcore::db::fast_level_map<ContractName, ContractIdEntry>                  _contract_name_to_id;
				dfdcore::db::fast_level_map<TransactionIdType, ResultTIdEntry>		  _request_to_result_iddb;
				dfdcore::db::fast_level_map<TransactionIdType, RequestIdEntry>		  _result_to_request_iddb;
				dfdcore::db::fast_level_map<TransactionIdType, ContractinTrxEntry>		  _trx_to_contract_iddb;
				dfdcore::db::fast_level_map<ContractIdType,ContractTrxEntry>		  _contract_to_trx_iddb;
...
FC_REFLECT_TYPENAME(std::vector<dfdcore::consensus::BlockIdType>)
FC_REFLECT_TYPENAME(std::unordered_set<dfdcore::consensus::TransactionIdType>)
FC_REFLECT(dfdcore::consensus::fee_index, (_fees)(_trx))
FC_REFLECT(dfdcore::consensus::data_version, (id)(version))
