...
namespace dfdcore {
    namespace consensus {
        ...
        struct MemoData
        {
             ...
             */
            fc::array<char, DFD_BLOCKCHAIN_MAX_MEMO_SIZE>     message;
        ...
        struct ExtendedMemoData
        {
            ...
            fc::array<char, DFD_BLOCKCHAIN_MAX_MEMO_SIZE>      message;
            fc::enum_type<uint8_t, MemoFlagsEnum>            memo_flags;
            fc::array<char, DFD_BLOCKCHAIN_EXTENDED_MEMO_SIZE> extra_memo;
        };
...
namespace fc {
    void to_variant(const dfdcore::consensus::WithdrawCondition& var, variant& vo);
    void from_variant(const variant& var, dfdcore::consensus::WithdrawCondition& vo);
    void to_variant(const dfdcore::consensus::MemoData& var, variant& vo);
    void from_variant(const variant& var, dfdcore::consensus::MemoData& vo);
}

FC_REFLECT_ENUM(dfdcore::consensus::WithdrawConditionTypes,
    (withdraw_null_type)
    (withdraw_signature_type)
    (withdraw_multisig_type)
    (withdraw_escrow_type)
    )
    FC_REFLECT_ENUM(dfdcore::consensus::WithdrawBalanceTypes,
    (withdraw_common_type)
    (withdraw_contract_type)
    (withdraw_margin_type)
    )
    FC_REFLECT(dfdcore::consensus::WithdrawCondition,
    (asset_id)
    (slate_id)
    (type)
    (balance_type)
    ...
    FC_REFLECT(dfdcore::consensus::TransferMemo,
    (one_time_key)
    (encrypted_memo_data)
    )
    FC_REFLECT_ENUM(dfdcore::consensus::MemoFlagsEnum,
    (from_memo)
    (to_memo)
    )
    FC_REFLECT(dfdcore::consensus::MemoData,
    (from)
    (from_signature)
    (message)
    (memo_flags)
    ...
    FC_REFLECT(dfdcore::consensus::ExtendedMemoData,
    (from)
    (from_signature)
    (message)
    (memo_flags)
    ...
    FC_REFLECT_DERIVED(dfdcore::consensus::MemoStatus,
    (dfdcore::consensus::MemoData),
    (has_valid_signature)
    (owner_private_key)
    )
    FC_REFLECT(dfdcore::consensus::WithdrawWithSignature,
    (owner)
    (memo)
    )

    FC_REFLECT(dfdcore::consensus::WithdrawWithMultisig,
    (required)
    (owners)
    (memo)
    )
    FC_REFLECT(dfdcore::consensus::withdraw_with_escrow,
    (sender)
    (receiver)
    (escrow)
    (agreement_digest)
