...
FC_REFLECT_ENUM(dfdcore::consensus::AccountType,
    (titan_account)
    (public_account)
    (multisig_account)
    )
    FC_REFLECT(dfdcore::consensus::MultisigMetaInfo,
    (required)
    (owners)
    )
    FC_REFLECT(dfdcore::consensus::AccountMetaInfo,
    (type)
    (data)
    )
    FC_REFLECT(dfdcore::consensus::DelegateStats,
    (votes_for)
    (pay_rate)
    (signing_key_history)
    ...
    FC_REFLECT(dfdcore::consensus::VoteDel,
    (votes)
    (delegate_id)
    )
    FC_REFLECT(dfdcore::consensus::AccountEntry,
    (id)
    (name)
    (public_data)
    (owner_key)
    ...
    FC_REFLECT(dfdcore::consensus::DelegatePaySalary, (total_balance)(pay_balance))
