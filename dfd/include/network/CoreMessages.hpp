...
namespace dfdcore {
    namespace network {
        ...
        const uint32_t core_protocol_version = DFD_NET_PROTOCOL_VERSION;
...
FC_REFLECT_ENUM(dfdcore::network::CoreMessageTypeEnum,
    (trx_message_type)
    (block_message_type)
    ...
    FC_REFLECT(dfdcore::network::ItemId, (item_type)
    (item_hash))
    FC_REFLECT(dfdcore::network::ItemIdsInventoryMessage, (item_type)
    (item_hashes_available))
    FC_REFLECT(dfdcore::network::BlockchainItemIdsInventoryMessage, (total_remaining_item_count)
    (item_type)
    (item_hashes_available))
    FC_REFLECT(dfdcore::network::FetchBlockchainItemIdsMessage, (item_type)
    (blockchain_synopsis))
    FC_REFLECT(dfdcore::network::FetchItemsMessage, (item_type)
    (items_to_fetch))
    FC_REFLECT(dfdcore::network::ItemNotAvailableMessage, (requested_item))
    FC_REFLECT(dfdcore::network::HelloMessage, (user_agent)
    ...
    FC_REFLECT_EMPTY(dfdcore::network::ConnectionAcceptedMessage)
    FC_REFLECT_ENUM(dfdcore::network::RejectionReasonCode, (unspecified)
    (different_chain)
    (already_connected)
    (connected_to_self)
    (not_accepting_connections)
    ...
    (invalid_hello_message)
    (client_too_old))
    FC_REFLECT(dfdcore::network::ConnectionRejectedMessage, (user_agent)
    (core_protocol_version)
    (remote_endpoint)
    (reason_code)
    (reason_string))
    FC_REFLECT_EMPTY(dfdcore::network::AddressRequestMessage)
    FC_REFLECT(dfdcore::network::AddressInfo, (remote_endpoint)
    (last_seen_time)
    (latency)
    (node_id)
    (direction)
    (firewalled))
    FC_REFLECT(dfdcore::network::AddressMessage, (addresses))
    FC_REFLECT(dfdcore::network::ClosingConnectionMessage, (reason_for_closing)
    (closing_due_to_error)
    (error))
    FC_REFLECT_ENUM(dfdcore::network::PeerConnectionDirection, (unknown)
    (inbound)
    (outbound))
    FC_REFLECT_ENUM(dfdcore::network::FirewalledState, (unknown)
    (firewalled)
    (not_firewalled))

    FC_REFLECT(dfdcore::network::CurrentTimeRequestMessage, (request_sent_time))
    FC_REFLECT(dfdcore::network::CurrentTimeReplyMessage, (request_sent_time)
    (request_received_time)
    (reply_transmitted_time))
    FC_REFLECT_ENUM(dfdcore::network::FirewallCheckResult, (unable_to_check)
    (unable_to_connect)
    (connection_successful))
    FC_REFLECT(dfdcore::network::CheckFirewallMessage, (node_id)(endpoint_to_check))
    FC_REFLECT(dfdcore::network::CheckFirewallReplyMessage, (node_id)(endpoint_checked)(result))
    FC_REFLECT_EMPTY(dfdcore::network::GetCurrentConnectionsRequestMessage)
    FC_REFLECT(dfdcore::network::CurrentConnectionData, (connection_duration)
    (remote_endpoint)
    (node_id)
    (clock_offset)
    (round_trip_delay)
    ...
    (firewalled)
    (user_data))
    FC_REFLECT(dfdcore::network::GetCurrentConnectionsReplyMessage, (upload_rate_one_minute)
...
{
    template<>
    struct hash < dfdcore::network::ItemId >
    {
        size_t operator()(const dfdcore::network::ItemId& item_to_hash) const
        {
