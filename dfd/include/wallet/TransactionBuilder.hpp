...
namespace dfdcore {
    namespace wallet {
        ...
        struct TransactionBuilder {
            ...
            std::unordered_set<dfdcore::consensus::Address>                                      required_signatures;

            ///Map of <owning account address, asset ID> to that account's balance in that asset ID
            std::map<std::pair<dfdcore::consensus::Address, AssetIdType>, ShareType>          outstanding_balances;

            ///Map of account address to key owning that account's market transactions
            std::map<dfdcore::consensus::Address, PublicKeyType>                               order_keys;
             ...
             */
            PublicKeyType order_key_for_account(const dfdcore::consensus::Address& account_address, const string& account_name);
             ...
             */
            /// @{
            void deduct_balance(const dfdcore::consensus::Address& account_to_charge, const dfdcore::consensus::Asset& amount)
            {
            ...
            void credit_balance(const dfdcore::consensus::Address& account_to_credit, const dfdcore::consensus::Asset& amount)
            {
...
} //namespace dfdcore::wallet

FC_REFLECT_ENUM(dfdcore::wallet::VoteStrategy,
    (vote_none)
    (vote_all)
    (vote_random)
    (vote_recommended)
    )
    FC_REFLECT(dfdcore::wallet::TransactionBuilder, (transaction_entry)(required_signatures)(outstanding_balances))
