...
namespace dfdcore {
    namespace wallet {
        ...
        enum PropertyEnum
        {
            ...
            last_scanned_block_number_for_dfdcore,
            is_forked
        };
        ...
        struct AccountData : public dfdcore::consensus::AccountEntry
        {
...
{
    template<>
    struct hash < dfdcore::wallet::ScriptRelationKey >
    {
        size_t operator()(const dfdcore::wallet::ScriptRelationKey& s)const
        {
...
FC_REFLECT_ENUM(dfdcore::wallet::WalletEntryTypeEnum,
    ...
    FC_REFLECT_ENUM(dfdcore::wallet::PropertyEnum,
    (version)
    (next_entry_number)
    (next_child_key_index)
    (automatic_backups)
...
(is_forked)
    ...
    FC_REFLECT(dfdcore::wallet::WalletProperty,
    (key)
    (value)
    )

    FC_REFLECT_DERIVED(dfdcore::wallet::AccountData, (dfdcore::consensus::AccountEntry),
    (is_my_account)
    (approved)
    (is_favorite)
    ...
    FC_REFLECT_DERIVED(dfdcore::wallet::AccountAddressData, (dfdcore::wallet::AccountData),
    (owner_address)
    (active_address)
    )

    FC_REFLECT(dfdcore::wallet::MasterKey,
    (encrypted_key)
    (checksum)
    )

    FC_REFLECT(dfdcore::wallet::KeyData,
    (account_address)
    (public_key)
    ...
    FC_REFLECT_ENUM(dfdcore::wallet::ContactData::ContactTypeEnum,
    (account_name)
    (public_key)
    (address)
    (btc_address)
    ...
    FC_REFLECT(dfdcore::wallet::ContactData,
    (contact_type)
    (data)
    (label)
    (favorite)
    ...
    FC_REFLECT(dfdcore::wallet::TransactionData,
    (entry_id)
    (block_num)
    (is_virtual)
    (is_confirmed)
    ...
    FC_REFLECT(dfdcore::wallet::LedgerEntry,
    (from_account)
    (to_account)
    ...
    FC_REFLECT(dfdcore::wallet::TransactionLedgerEntry,
    (id)
    (block_num)
    (block_id)
    (timestamp)
    ...
    FC_REFLECT_DERIVED(dfdcore::wallet::PrettyTransactionExperimental, (dfdcore::wallet::TransactionLedgerEntry),
    (inputs)
    (outputs)
    (balances)
    (notes)
    ...
    FC_REFLECT(dfdcore::wallet::Setting,
    (name)
    (value)
    )

    FC_REFLECT(dfdcore::wallet::GenericWalletEntry,
    (type)
    (data)
    )

    FC_REFLECT(dfdcore::wallet::ScriptEntry,
    (id)
    (enable)
    (code)
    (description)
    ...
    FC_REFLECT(dfdcore::wallet::ScriptEntryPrintable,
    (id)
    (enable)
    (code_printable)
    (description)
    ...
    FC_REFLECT(dfdcore::wallet::ScriptRelationKey,
    (contract_id)
    (event_type)
    )
...
namespace fc {
    ...
    template<typename T, dfdcore::wallet::WalletEntryTypeEnum N>
    struct get_typename < dfdcore::wallet::WalletEntry<T, N> >
    {
    ...
    template<typename Type, dfdcore::wallet::WalletEntryTypeEnum N>
    struct reflector < dfdcore::wallet::WalletEntry<Type, N> >
    {
        typedef dfdcore::wallet::WalletEntry<Type, N>  type;
        ...
        {
            {
                typedef decltype(((dfdcore::wallet::BaseEntry<N>*)nullptr)->wallet_entry_index) member_type;
                visitor.TEMPLATE operator() < member_type, dfdcore::wallet::BaseEntry<N>, &dfdcore::wallet::BaseEntry<N>::wallet_entry_index > ("index");
            }
