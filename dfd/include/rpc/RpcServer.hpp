...
namespace dfdcore {
    namespace rpc {
        ...
        /**
        *  @class rpc_server
        *  @brief provides a json-rpc interface to the dfdcore client
        */
        class RpcServer
        {
        public:
            RpcServer(dfdcore::client::Client* client);
            virtual ~RpcServer();

            bool        configure_rpc(const dfdcore::client::RpcServerConfig& cfg);
            bool        configure_http(const dfdcore::client::RpcServerConfig& cfg);
            bool        configure_encrypted_rpc(const dfdcore::client::RpcServerConfig& cfg);
            ...
            const dfdcore::api::MethodData& get_method_data(const std::string& method_name);
            std::vector<dfdcore::api::MethodData> get_all_method_data() const;
        ...
        protected:
            friend class dfdcore::rpc::detail::RpcServerImpl;

            void validate_method_data(dfdcore::api::MethodData method);
            void register_method(dfdcore::api::MethodData method);
