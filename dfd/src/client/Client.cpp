...
//#include <dfdcore/mail/server.hpp>
...
namespace dfdcore {
    namespace client {
        ...
        const string DFD_MESSAGE_MAGIC = "Dfd Signed Message:\n";
        ...
        program_options::variables_map parse_option_variables(int argc, char** argv)
        {
            ...
            option_config.add_options()
                ...
                ("growl-identifier", program_options::value<std::string>(), "A name displayed in growl messages to identify this dfd_client instance")
                ;
            ...
            {
                std::cout << fc::json::to_pretty_string(dfdcore::client::version_info()) << "\n";
                exit(0);
            }
        ...
        fc::path get_data_dir(const program_options::variables_map& option_variables)
        {
            try {
                ...
                if (option_variables.count("data-dir"))
                {
					...
					if (dfdcore::utilities::isGBK(temp_data_dir.data()))
					{
						temp_data_dir = dfdcore::console::GBKToUTF8(temp_data_dir);
					}
...
#ifdef DFD_TEST_NETWORK
                        //dir_name += "-Test" + std::to_string(DFD_TEST_NETWORK_VERSION);
                        dir_name += "-Test";
#endif
                        return dir_name;
                    };

                    datadir = fc::app_path() / get_os_specific_dir_name(DFD_BLOCKCHAIN_NAME);
                }
                return datadir;
        ...
        void load_checkpoints(const fc::path& data_dir)
        {
            try {
                ...
                decltype(dfdcore::consensus::detail::CHECKPOINT_BLOCKS) external_checkpoints;
                ...
                {
                    decltype(dfdcore::consensus::detail::CHECKPOINT_BLOCKS) external_checkpoints;
                    if (dfdcore::consensus::detail::CHECKPOINT_BLOCKS.empty() || external_checkpoints.crbegin()->first >= dfdcore::consensus::detail::CHECKPOINT_BLOCKS.crbegin()->first)
                    {
                        ulog("Using blockchain checkpoints from file: ${x}", ("x", checkpoint_file.preferred_string()));
                        dfdcore::consensus::detail::CHECKPOINT_BLOCKS = external_checkpoints;
                        return;
                    }
                }
                ...
                if (!file_exception.valid())
                {
                    ...
                    fc::json::save_to_file(dfdcore::consensus::detail::CHECKPOINT_BLOCKS, checkpoint_file);
                }
                else
                {
                ...
                if (!dfdcore::consensus::detail::CHECKPOINT_BLOCKS.empty())
        ...
        namespace detail
        {
                ...
                const program_options::variables_map& option_variables)
            {
                if (option_variables.count("server") || option_variables.count("daemon") || cfg.rpc.enable)
                {
                    ...
                    // dfdcore::rpc::rpc_server::config rpc_config(cfg.rpc);
            ...
            void ClientImpl::start_delegate_loop()
            {
                ...
                if (!_time_discontinuity_connection.connected())
                    _time_discontinuity_connection = dfdcore::consensus::time_discontinuity_signal.connect([=](){ reschedule_delegate_loop(); });
            ...
            void ClientImpl::delegate_loop()
            {
                ...
                {
                    set_target_connections(DFD_NET_DELEGATE_DESIRED_CONNECTIONS);
                ...
                if (next_block_time.valid())
                {
                    ...
                    if (*next_block_time <= now)
                    {
                        try
                        {
                            ...
                            FC_ASSERT((now - *next_block_time) < fc::seconds(DFD_BLOCKCHAIN_BLOCK_INTERVAL_SEC),
                ...
                uint32_t slot_number = dfdcore::consensus::get_slot_number(now);
                time_point_sec next_slot_time = dfdcore::consensus::get_slot_start_time(slot_number + 1);
                ilog("Rescheduling delegate loop for time: ${t}", ("t", next_slot_time));

                time_point scheduled_time = next_slot_time;
                if (dfdcore::consensus::ntp_time().valid())
                    scheduled_time -= dfdcore::consensus::ntp_error();
            ...
            void ClientImpl::start_rebroadcast_pending_loop()
            {
                if (!_rebroadcast_pending_loop_done.valid() || _rebroadcast_pending_loop_done.ready())
                    ...
                    fc::time_point::now() + fc::seconds((int64_t)(DFD_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 1.3)),
                    "rebroadcast_pending");
            }
            void ClientImpl::start_fork_update_time_loop()
            {
                if (!_fork_update_loop_done.valid() || _fork_update_loop_done.ready())
                    ...
                    fc::time_point::now() + fc::seconds((int64_t)(DFD_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 6)),
                    "rebroadcast_pending");
            }
            void ClientImpl::fork_update_time_loop()
            {
                ...
                {
                    dfdcore::consensus::update_ntp_time();
                    _chain_db->set_forkdb_num(fork_num);
                }
            }
            dfdcore::consensus::PublicKeyType ClientImpl::wallet_get_account_owner_publickey(const std::string& account_name)
            {
                // set limit in  simulator state
                if (_chain_db->get_is_in_simulator())
                    FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");
            ...
            void ClientImpl::rebroadcast_pending_loop()
            {
                ...
                else
                {
                    try
                    {
                        ...
                        std::vector<dfdcore::consensus::SignedTransaction> temp_vec;
                    ...
                    try
                    {
                        ...
                        if (fork_data && fork_data->is_known)
                        {
                            if (sync_mode && !fork_data->is_linked)
                                FC_THROW_EXCEPTION(dfdcore::consensus::unlinkable_block,
                        ...
                        else
                        {
                            ...
                            if (sync_mode && !result.is_linked)
                                FC_THROW_EXCEPTION(dfdcore::consensus::unlinkable_block, "The blockchain accepted this block, but it isn't linked");
            ...
            bool ClientImpl::has_item(const dfdcore::network::ItemId& id)
            {
            ...
            */
            bool ClientImpl::handle_message(const dfdcore::network::Message& message_to_handle, bool sync_mode)
            {
                if (g_client_quit)
                    return true;
                ...
                try
                {
                    ...
                    case block_message_type:
                    {
                        ...
                        if (block_message_to_handle.block.block_size() > DFD_BLOCKCHAIN_MAX_BLOCK_SIZE)
                        {
                        ...
                        dfdcore::consensus::BlockIdType old_head_block = _chain_db->get_head_block_id();
                    ...
                    case trx_message_type:
                    {
                        ...
                        if (trx_message_to_handle.trx.data_size() > DFD_BLOCKCHAIN_MAX_TRX_SIZE)
                        {
                ...
                catch (const dfdcore::consensus::insufficient_relay_fee& original_exception)
                {
                    FC_THROW_EXCEPTION(dfdcore::network::insufficient_relay_fee, "Insufficient relay fee; do not propagate!",
                        ("original_exception", original_exception.to_detail_string()));
                }
                catch (const dfdcore::consensus::block_older_than_undo_history& original_exception)
                {
                    FC_THROW_EXCEPTION(dfdcore::network::block_older_than_undo_history, "Block is older than undo history, stop fetching blocks!",
                        ("original_exception", original_exception.to_detail_string()));
                }
            }
            ...
            */
            std::vector<dfdcore::network::ItemHashType> ClientImpl::get_item_ids(uint32_t item_type,
                const std::vector<dfdcore::network::ItemHashType>& blockchain_synopsis,
                uint32_t& remaining_item_count,
                uint32_t limit /* = 2000 */)
            {
                // limit = 20; // for testing
                FC_ASSERT(item_type == dfdcore::client::block_message_type);
                ...
                dfdcore::network::ItemHashType last_seen_block_hash;
                for (const dfdcore::network::ItemHashType& item_hash : boost::adaptors::reverse(blockchain_synopsis))
                {
                    try
                    {
                ...
                std::vector<dfdcore::network::ItemHashType> hashes_to_return;
            ...
            std::vector<dfdcore::network::ItemHashType> ClientImpl::get_blockchain_synopsis(uint32_t item_type,
            ...
            {
                FC_ASSERT(item_type == dfdcore::client::block_message_type);
                std::vector<dfdcore::network::ItemHashType> synopsis;
                uint32_t high_block_num = 0;
                uint32_t non_fork_high_block_num = 0;
                std::vector<BlockIdType> fork_history;

                if (reference_point != dfdcore::network::ItemHashType())
                {
            ...
            dfdcore::network::Message ClientImpl::get_item(const dfdcore::network::ItemId& id)
            {
                if (id.item_type == block_message_type)
                {
                    //   uint32_t block_number = _chain_db->get_block_num(id.item_hash);
                    dfdcore::client::BlockMessage block_message_to_send(_chain_db->get_block(id.item_hash));
            ...
            fc::path ClientImpl::script_compile(const fc::path& filename) const
            {
                ...
                try{
                    ...
                    {
                        FC_THROW_EXCEPTION(dfdcore::consensus::invalid_script_source_filename, "script source file name should end with .lua or .glua");
                    }
					auto p_lua_module = std::make_shared<UvmModuleByteStream>();
                    FC_ASSERT(p_lua_module, "Alloc memory for UvmModuleByteStream failed!");
					::dfdcore::consensus::contract_engine::ContractEngineBuilder builder;
                    ...
                    {
                        FC_THROW_EXCEPTION(dfdcore::consensus::save_bytecode_to_scriptfile_fail, err_msg);
                    }
            ...
            uint32_t ClientImpl::get_block_number(const dfdcore::network::ItemHashType& block_id)
            {
                return _chain_db->get_block_num(block_id);
            }

            fc::time_point_sec ClientImpl::get_block_time(const dfdcore::network::ItemHashType& block_id)
            {
                if (block_id == dfdcore::network::ItemHashType())
                {
            ...
            fc::time_point_sec ClientImpl::get_blockchain_now()
            {
                ...
                return dfdcore::consensus::nonblocking_now();
            }

            dfdcore::network::ItemHashType ClientImpl::get_head_block_id() const
            {
            ...
            {
                return dfdcore::consensus::estimate_last_known_fork_from_git_revision_timestamp(unix_timestamp);
            }
            ...
            void ClientImpl::blocks_too_old_monitor_task()
            {
                ...
                if (_chain_db->get_head_block().timestamp < dfdcore::consensus::now() - fc::seconds(DFD_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 2) &&
                    !_sync_mode &&
                ...
                if (!_blocks_too_old_monitor_done.canceled())
                    ...
                    fc::time_point::now() + fc::seconds(DFD_BLOCKCHAIN_BLOCK_INTERVAL_SEC),
                    "block_monitor_task");
            }
        ...
        Client::Client(const std::string& user_agent,
            dfdcore::network::SimulatedNetworkPtr network_to_connect_to)
            : my(new detail::ClientImpl(this, user_agent))
        {
            ...
            const std::function<void(float)> replay_status_callback)
        {
            try {
                ...
                my->_wallet = std::make_shared<dfdcore::wallet::Wallet>(my->_chain_db, my->_config.wallet_enabled);
                my->_wallet->set_data_directory(wallet_dir / "wallets");

                //delete?
                //if (my->_config.mail_server_enabled)
                //{
                //my->_mail_server = std::make_shared<dfdcore::mail::server>();
                //my->_mail_server->open(data_dir / "mail");
                //}
                //my->_mail_client = std::make_shared<dfdcore::mail::client>(my->_wallet, my->_chain_db);
                ...
                if (!my->_p2p_node)
                    my->_p2p_node = std::make_shared<dfdcore::network::Node>(my->_user_agent);
        ...
        dfdcore::rpc::RpcServerPtr Client::get_rpc_server()const { return my->_rpc_server; }
        dfdcore::network::NodePtr Client::get_node()const { return my->_p2p_node; }

        fc::variant_object version_info()
        {
            string client_version(dfdcore::utilities::git_revision_description);
            const size_t pos = client_version.find('/');
            if (pos != string::npos && client_version.size() > pos)
                client_version = client_version.substr(pos + 1);
#ifdef DFD_TEST_NETWORK
            //client_version += "-testnet-" + std::to_string(DFD_TEST_NETWORK_VERSION);
			client_version += "-testnet-1.0.6";
#else
			client_version += "-1.0.6";
#endif

            fc::mutable_variant_object info;
            info["blockchain_name"] = DFD_BLOCKCHAIN_NAME;
            info["blockchain_description"] = DFD_BLOCKCHAIN_DESCRIPTION;
            info["client_version"] = client_version;
            info["dfd_revision"] = dfdcore::utilities::git_revision_sha;
            info["dfd_revision_age"] = fc::get_approximate_relative_time_string(fc::time_point_sec(dfdcore::utilities::git_revision_unix_timestamp));
...
#if defined(DFD_CLIENT_JENKINS_BUILD_NUMBER)
            info["jenkins_build_number"] = DFD_CLIENT_JENKINS_BUILD_NUMBER;
#endif
#if defined(DFD_CLIENT_JENKINS_BUILD_URL)
            info["jenkins_build_url"] = DFD_CLIENT_JENKINS_BUILD_URL;
#endif
        ...
        void Client::start_networking(std::function<void()> network_started_callback)
        {
            ...
            {
                dfdcore::network::ChainDownloader* chain_downloader = new dfdcore::network::ChainDownloader();
        ...
        void Client::configure_from_command_line(int argc, char** argv)
        {
            ...
            {
                my->_cli = new dfdcore::console::Cli(this, nullptr, &std::cout);
                return;
            }
            g_client = this;
            // parse command-line options
            ...
            fc::path datadir = dfdcore::client::get_data_dir(option_variables);
            fc::path walletDir = dfdcore::client::get_wallet_dir(option_variables, datadir);
            ...
            // else we use the default set in dfdcore::network::node
            ...
            else //we will accept input from the console
            {
                ...
                if (option_variables.count("log-commands") <= 0)
                {
                    ...
                    my->_cli = new dfdcore::console::Cli(this, my->_command_script_holder.get(), &std::cout);
                }
                else
                {
                    ...
                    my->_cli = new dfdcore::console::Cli(this, my->_command_script_holder.get(), my->_tee_stream.get());
            ...
            if (option_variables["upnp"].as<bool>())
            {
                ...
                my->_upnp_service = std::unique_ptr<dfdcore::network::UpnpService>(new dfdcore::network::UpnpService);
            ...
            start_networking([=]{
                ...
                else if (!option_variables.count("disable-default-peers"))
                {
                    for (string default_peer : my->_config.default_peers)
                    {
                        ...
                        catch (dfdcore::network::endpoint_in_blacklist)
                        {
                        }
                    }
                }
            ...
            {
                my->_chain_server = std::unique_ptr<dfdcore::network::ChainServer>(
                    new dfdcore::network::ChainServer(my->_chain_db,
        ...
        dfdcore::network::NodeIdType Client::get_node_id() const
        {
        ...
        void Client::add_node(const string& remote_endpoint, int32_t oper_flag)
        {
            ...
            catch (const dfdcore::network::already_connected_to_requested_peer&)
            {
            }
        }
        ...
        {
            dfdcore::network::ItemId head_item_id;
            head_item_id.item_type = dfdcore::client::block_message_type;
            uint32_t last_block_num = my->_chain_db->get_head_block_num();
            if (last_block_num == (uint32_t)-1)
                head_item_id.item_hash = dfdcore::network::ItemHashType();
            else
                head_item_id.item_hash = my->_chain_db->get_head_block_id();
            my->_p2p_node->sync_from(head_item_id, dfdcore::consensus::get_list_of_fork_block_numbers());
        ...
        bool Client::handle_message(const dfdcore::network::Message& message, bool sync_mode)
        {
        ...
        */
        ...
        dfdcore::api::CommonApi* Client::get_impl() const
        {
            return my.get();
        }
