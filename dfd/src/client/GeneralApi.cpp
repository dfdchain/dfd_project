...
namespace dfdcore {
    namespace client {
        namespace detail {
            ...
            {
                return dfdcore::client::version_info();
            }
            ...
            variant_object ClientImpl::get_info()const
            {
                ...
                const auto share_entry = _chain_db->get_asset_entry(DFD_BLOCKCHAIN_SYMBOL);
                if (share_entry.valid())
					info["blockchain_share_supply"] = share_entry->current_share_supply - DFD_BLOCKCHAIN_ASSET_SHADOW;

                const auto blocks_left = DFD_BLOCKCHAIN_NUM_DELEGATES - (head_block_num % DFD_BLOCKCHAIN_NUM_DELEGATES);
                ...
                if (head_block_num > 0)
                {
                    const auto current_round_timestamp = dfdcore::consensus::get_slot_start_time(now);
                    const auto next_round_timestamp = current_round_timestamp + (blocks_left * DFD_BLOCKCHAIN_BLOCK_INTERVAL_SEC);
                ...
                info["client_version"] = dfdcore::client::version_info()["client_version"].as_string();
				...
				*/
                ...
                {
                    info["ntp_time"] = now;
                    info["ntp_time_error"] = static_cast<double>(dfdcore::consensus::ntp_error().count()) / fc::seconds(1).count();
                }
            ...
            fc::variant_object ClientImpl::validate_address(const string& address) const
            {
                ...
                try
                {
                    dfdcore::consensus::PublicKeyType test_key(address);
                    result["isvalid"] = true;
                }
                ...
                const std::vector<fc::variants>& parameters_list) const
            {
                ...
                FC_ASSERT(_self->get_rpc_server()->get_method_data(method_name).prerequisites == dfdcore::api::no_prerequisites,
            ...
            {
                dfdcore::consensus::ntp_time(); // this is just called to ensure the NTP time service is running
