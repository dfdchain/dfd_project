    ...
    namespace client {
        namespace detail {
            ...
            int ClientImpl::save_code_to_file(const string& name, UvmModuleByteStream *stream, char* err_msg) const
            {
                ...
                for (int i = 0; i < 5; ++i)
                    dfdcore::utilities::common_fwrite_int(f, (int*)&digest[i]);

				int p_new_stream_buf_size = (int) p_new_stream->buff.size();
                dfdcore::utilities::common_fwrite_int(f, &p_new_stream_buf_size);
				p_new_stream->buff.resize(p_new_stream_buf_size);
                dfdcore::utilities::common_fwrite_stream(f, p_new_stream->buff.data(), p_new_stream->buff.size());

                int contract_apis_count = (int)p_new_stream->contract_apis.size();
                dfdcore::utilities::common_fwrite_int(f, &contract_apis_count);
                for (int i = 0; i < contract_apis_count; ++i)
                {
                    int api_len = p_new_stream->contract_apis[i].length();
                    dfdcore::utilities::common_fwrite_int(f, &api_len);
                    dfdcore::utilities::common_fwrite_stream(f, p_new_stream->contract_apis[i].c_str(), api_len);
                }

                int offline_apis_count = (int)p_new_stream->offline_apis.size();
                dfdcore::utilities::common_fwrite_int(f, &offline_apis_count);
                for (int i = 0; i < offline_apis_count; ++i)
                {
                    int offline_api_len = p_new_stream->offline_apis[i].length();
                    dfdcore::utilities::common_fwrite_int(f, &offline_api_len);
                    dfdcore::utilities::common_fwrite_stream(f, p_new_stream->offline_apis[i].c_str(), offline_api_len);
                }

                int contract_emit_events_count = p_new_stream->contract_emit_events.size();
                dfdcore::utilities::common_fwrite_int(f, &contract_emit_events_count);
                for (int i = 0; i < contract_emit_events_count; ++i)
                {
                    int event_len = p_new_stream->contract_emit_events[i].length();
                    dfdcore::utilities::common_fwrite_int(f, &event_len);
                    dfdcore::utilities::common_fwrite_stream(f, p_new_stream->contract_emit_events[i].c_str(), event_len);
                }

                int contract_storage_properties_count = p_new_stream->contract_storage_properties.size();
                dfdcore::utilities::common_fwrite_int(f, &contract_storage_properties_count);
                for (const auto& storage_info : p_new_stream->contract_storage_properties)
                {
                    int storage_len = storage_info.first.length();
                    dfdcore::utilities::common_fwrite_int(f, &storage_len);
                    dfdcore::utilities::common_fwrite_stream(f, storage_info.first.c_str(), storage_len);
                    int storage_type = storage_info.second;
                    dfdcore::utilities::common_fwrite_int(f, &storage_type);
                }
            ...
            ContractEntryPrintable ClientImpl::contract_load_to_file(const string &contract, const fc::path &file)
            {
                ...
                dfdcore::utilities::common_fwrite_int(f, &byte_code_len);
                dfdcore::utilities::common_fwrite_stream(f, contract_entry->code.byte_code.data(), byte_code_len);
                ...
                for (const auto& abi: contract_entry->code.abi)
                {
                    ...
                    dfdcore::utilities::common_fwrite_int(f, &api_len);
                    dfdcore::utilities::common_fwrite_stream(f, abi.c_str(), api_len);
                }
                ...
                for (const auto& offline_abi: contract_entry->code.offline_abi)
                {
                    ...
                    dfdcore::utilities::common_fwrite_int(f, &offline_api_len);
                    dfdcore::utilities::common_fwrite_stream(f, offline_abi.c_str(), offline_api_len);
                }
                ...
                for (const auto& event: contract_entry->code.events)
                {
                    ...
                    dfdcore::utilities::common_fwrite_int(f, &event_len);
                    dfdcore::utilities::common_fwrite_stream(f, event.c_str(), event_len);
                }
            ...
            dfdcore::consensus::Address ClientImpl::get_contract_address(const string& contract,ChainInterfacePtr db) const
            {
                if (db == nullptr)
                {
                    db = _chain_db;
            ...
            fc::path ClientImpl::contract_compile_helper(const fc::path& filename) const
            {
                ...
                {
                    FC_THROW_EXCEPTION(dfdcore::consensus::invalid_contract_filename, "contract source file name should end with .lua or .glua");
                }
                ...
                if (false)
                {
                    ...
                    FC_THROW_EXCEPTION(dfdcore::consensus::contract_compile_fail, err_msg);
                ...
                if (save_code_to_file(out_filename, p_lua_module, err_msg) < 0)
                {
					...
					err_msg[LUA_EXCEPTION_MULTILINE_STRNG_MAX_LENGTH] = '\0';
                    FC_THROW_EXCEPTION(dfdcore::consensus::save_bytecode_to_gpcfile_fail, err_msg);
                }
				if(p_lua_module)
					delete p_lua_module;
            ...
            std::vector<dfdcore::consensus::Asset> ClientImpl::contract_register_testing(
                const std::string& owner,
                const fc::path& codefile)
            {
				// set limit in  simulator state
            ...
            dfdcore::wallet::WalletTransactionEntry ClientImpl::contract_call(const std::string& contract, const std::string& caller_name, const std::string& function_name, const std::string& params, const std::string& asset_symbol, const fc::optional<double>& call_limit)
            {
				// set limit in  simulator state
				if (_chain_db->get_is_in_simulator())
					FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");
            ...
            std::vector<dfdcore::consensus::Asset> ClientImpl::contract_call_testing(const std::string& contract, const std::string& caller_name, const std::string& function_name, const std::string& params)
            {
				// set limit in  simulator state
				if (_chain_db->get_is_in_simulator())
					FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");
            ...
            dfdcore::wallet::WalletTransactionEntry ClientImpl::contract_upgrade(
			...
			std::vector<dfdcore::consensus::Asset> ClientImpl::contract_upgrade_testing(
				const std::string& contract_address,
				const std::string& upgrader_name,
				const std::string& new_contract_name,
				const std::string& new_contract_desc
            ...
            dfdcore::wallet::WalletTransactionEntry ClientImpl::contract_destroy(
                const std::string& contract_address,
			...
			std::vector<dfdcore::consensus::Asset> ClientImpl::contract_destroy_testing(const std::string& contract_address, const std::string& destroyer_name)
			{
				// set limit in  simulator state
				if (_chain_db->get_is_in_simulator())
					FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");
            ...
            dfdcore::consensus::ContractEntryPrintable ClientImpl::contract_get_info(const std::string& contract)
            {
				// set limit in  simulator state
				if (_chain_db->get_is_in_simulator())
					FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");
                ...
                dfdcore::consensus::oContractEntry entry = _chain_db->get_contract_entry(contract_address);
                FC_ASSERT(entry.valid());

                ContractEntryPrintable print_entry(*entry);

                dfdcore::consensus::oAccountEntry account_entry = _chain_db->get_account_entry(print_entry.owner_address);
            ...
            std::vector<dfdcore::consensus::BalanceEntry> ClientImpl::contract_get_balance(const std::string& contract)
            {
				// set limit in  simulator state
				if (_chain_db->get_is_in_simulator())
					FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");

                std::vector<dfdcore::consensus::BalanceEntry> entry_list;
            ...
            std::vector<dfdcore::consensus::Asset> ClientImpl::wallet_transfer_to_contract_testing(
                const string& amount_to_transfer,
                const string& asset_symbol,
                const string& from_account_name,
			...
			dfdcore::consensus::TransactionIdType ClientImpl::get_result_trx_id(const dfdcore::consensus::TransactionIdType& id)
			{
				// set limit in  simulator state
				if (_chain_db->get_is_in_simulator())
					FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");
			...
			dfdcore::consensus::TransactionIdType ClientImpl::get_request_trx_id(const dfdcore::consensus::TransactionIdType& id)
			{
				// set limit in  simulator state
				if (_chain_db->get_is_in_simulator())
					FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");
			...
			std::string ClientImpl::get_contract_registered_in_transaction(const dfdcore::consensus::TransactionIdType& trx_id)
			{
			...
			dfdcore::consensus::TransactionIdType ClientImpl::get_transaction_id_contract_registered(const std::string& contract_id)
			{
