...
namespace dfdcore {
    namespace client {
        namespace detail {
            ...
            std::vector<dfdcore::consensus::AccountEntry> ClientImpl::wallet_get_all_approved_accounts(int8_t approval)
            {
                // set limit in  simulator state
                if (_chain_db->get_is_in_simulator())
                    FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");
            ...
            void detail::ClientImpl::wallet_create(const string& wallet_name, const string& password, const string& brain_key)
            {
                ...
                if (brain_key.size() && brain_key.size() < DFD_WALLET_MIN_BRAINKEY_LENGTH) FC_CAPTURE_AND_THROW(brain_key_too_short);
                if (password.size() < DFD_WALLET_MIN_PASSWORD_LENGTH) FC_CAPTURE_AND_THROW(password_too_short);
            ...
            void read_keys(const fc::variant& vo, vector<PrivateKeyType>& keys, const string& password)
            {
                ...
                try
                {
                    ...
                    const auto key = dfdcore::utilities::wif_to_key(wif_key);
                ...
                catch (const dfdcore::wallet::invalid_password&)
                {
                    throw;
                }
                catch (...)
            ...
            */
            ...
            vector<dfdcore::wallet::PrettyTransaction> detail::ClientImpl::wallet_transaction_history_splite(const std::string& account_name,
                ...
                int32_t transaction_type)const
            {
                ...
                try {
                    const auto history = _wallet->get_pretty_transaction_history(account_name, 0, -1, asset_symbol, (dfdcore::wallet::Wallet::TrxType)transaction_type, true);
            ...
            bool detail::ClientImpl::wallet_check_address(const std::string& address)
            {
                ...
                size_t first = address.find_first_of(DFD_ADDRESS_PREFIX);
            ...
            map<TransactionIdType, fc::exception> detail::ClientImpl::wallet_get_pending_transaction_errors(const dfdcore::consensus::FilePath& filename)const
            {
                // set limit in  simulator state
                if (_chain_db->get_is_in_simulator())
                    FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");
            ...
            WalletTransactionEntry detail::ClientImpl::sign_build_transaction(const dfdcore::wallet::WalletTransactionEntry& trasaction_building)
            {
			...
			std::string detail::ClientImpl::broadcast_building_transaction(const dfdcore::wallet::WalletTransactionEntry& trasaction_building)
            {
                WalletTransactionEntry trx_entry = trasaction_building;
			...
			fc::variant_object detail::ClientImpl::wallet_builder_get_multisig_detail(const dfdcore::wallet::TransactionBuilder& transaction_builder) const
			{
				try {
			...
			fc::variant_object detail::ClientImpl::wallet_builder_file_get_multisig_detail(const dfdcore::consensus::FilePath& builder_path) const
			{
				try {
					auto new_builder = _wallet->create_transaction_builder_from_file(builder_path);
			...
			fc::variant_object detail::ClientImpl::wallet_import_multisig_account(
				const dfdcore::consensus::Address& multisig_address)
			{
			...
			Address detail::ClientImpl::wallet_import_multisig_account_by_detail(
				const string& asset_symbol,
				uint32_t m,
				const vector<dfdcore::consensus::Address>& addresses)
			{
			...
			std::pair<std::string, dfdcore::wallet::WalletTransactionEntry> detail::ClientImpl::wallet_create_multisig_account(
				const string& amount,
				const string& symbol,
				const string& from_name,
				uint32_t m,
				const std::vector<dfdcore::consensus::Address>& addresses,
				const Imessage& memo_message)
			{
				Asset ugly_asset = _chain_db->to_ugly_asset(amount, symbol);
				auto builder = _wallet->create_transaction_builder();
			...
			std::vector<dfdcore::wallet::PrettyTransaction> detail::ClientImpl::wallet_multisig_account_history(const std::string& account_address, const std::string& asset_symbol, int32_t limit, uint32_t start_block_num, uint32_t end_block_num) const
			{
				try {
					Address multisig_address(account_address);
					if (multisig_address.judge_addr_type(account_address) != AddressType::multisig_address)
						return std::vector<dfdcore::wallet::PrettyTransaction>();
            ...
            vector<dfdcore::wallet::AccountAddressData> detail::ClientImpl::wallet_list_my_addresses() const
            {
                // set limit in  simulator state
                if (_chain_db->get_is_in_simulator())
                    FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");
            ...
            // vector<dfdcore::wallet::escrow_summary> ClientImpl::wallet_escrow_summary( const string& account_name ) const
            // { try {
            ...
            std::map<std::string, dfdcore::consensus::DelegatePaySalary>  ClientImpl::wallet_active_delegate_salary()
            {
                // set limit in  simulator state
                if (_chain_db->get_is_in_simulator())
                    FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");
            ...
            void ClientImpl::wallet_set_transaction_imessage_fee_coe(const string& fee_coe)
            {
                ...
                try
                {
                    ...
                    if (ipos != string::npos)
                    {
                        ...
                        FC_ASSERT((precision_input <= DFD_BLOCKCHAIN_PRECISION), "Precision is not correct");
                    }
                   
                    Asset asset_to_transfer = _chain_db->to_ugly_asset(fee_coe, DFD_BLOCKCHAIN_SYMBOL);
            ...
            double ClientImpl::wallet_get_transaction_imessage_fee_coe()
            {
                ...
                return ((double)fee_coe) / DFD_BLOCKCHAIN_PRECISION;
            }
            ...
            Asset ClientImpl::wallet_set_transaction_fee(const string& fee)
            {
                ...
                try {
                    ...
                    Asset asset_to_transfer = _chain_db->to_ugly_asset(fee, DFD_BLOCKCHAIN_SYMBOL);
            ...
            Asset ClientImpl::wallet_get_transaction_fee(const string& fee_symbol)
            {
                ...
                if (fee_symbol.empty())
                    return _wallet->get_transaction_fee(_chain_db->get_asset_id(DFD_BLOCKCHAIN_SYMBOL));
            ...
            std::string ClientImpl::wallet_transfer_to_address_rpc(const std::string& amount_to_transfer, const std::string& asset_symbol, const std::string& from_account_name, const std::string& to_address, const dfdcore::consensus::Imessage& memo_message /* = fc::json::from_string("").as<std::string>() */, const dfdcore::wallet::VoteStrategy& strategy /* = fc::json::from_string("vote_recommended").as<dfdcore::wallet::vote_strategy>() */)
            {
                // set limit in  simulator state
                if (_chain_db->get_is_in_simulator())
                    FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");
            ...
            std::string detail::ClientImpl::wallet_transfer_to_public_account_rpc(
                ...
                const string& from_account_name,
                const string& to_account_name,
                const dfdcore::consensus::Imessage& memo_message,
                const dfdcore::wallet::VoteStrategy& strategy)
            {
                // set limit in  simulator state
                if (_chain_db->get_is_in_simulator())
                    FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");
            ...
            string ClientImpl::wallet_account_balance_rpc(const string& account_name)const
            {
                ...
                try {
                    ...
                    else
                    {
                        ...
                        auto resu = re_val.find(dfdcore::consensus::Asset(0, 0).asset_id);
                        if (resu == re_val.end())
                        {
                            res += "0\"}";
                        }
