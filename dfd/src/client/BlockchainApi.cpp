    ...
    namespace client {
        namespace detail {
            ...
            vector<AccountEntry> ClientImpl::blockchain_list_active_delegates(uint32_t first, uint32_t count)const
            {
                ...
                if (first > 0) --first;
                FC_ASSERT(first < DFD_BLOCKCHAIN_NUM_DELEGATES, "Only the first 9 delegates are active");
                FC_ASSERT(first + count <= DFD_BLOCKCHAIN_NUM_DELEGATES, "Only the first 9 delegates are active");
                vector<AccountIdType> all_delegate_ids = _chain_db->get_active_delegates();
                FC_ASSERT(all_delegate_ids.size() == DFD_BLOCKCHAIN_NUM_DELEGATES, "The num of active delegates should equal to DFD_BLOCKCHAIN_NUM_DELEGATES");
            ...
            vector<string> ClientImpl::blockchain_list_missing_block_delegates(uint32_t block_num)
            {
                ...
                timestamp += DFD_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                while (timestamp != this_block.timestamp)
                    // if time interval between prev_block and this block greater than DFD_BLOCKCHAIN_BLOCK_INTERVAL_SEC,
            ...
            vector<DfdTrxidBalance> ClientImpl::blockchain_get_dfd_account_balance_entry(const uint32_t block_num)
            {
                // set limit in  simulator state
                if (_chain_db->get_is_in_simulator())
                    FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");
                ...
                auto results = _chain_db->fetch_dfd_full_entry(block_num, last_scan_num);
            ...
            std::vector<std::pair<dfdcore::consensus::TransactionIdType, dfdcore::consensus::SignedTransaction>>  ClientImpl::blockchain_list_pending_transactions() const
            {
                // set limit in  simulator state
                if (_chain_db->get_is_in_simulator())
                    FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");

                std::vector<std::pair<dfdcore::consensus::TransactionIdType, dfdcore::consensus::SignedTransaction>> trxs;
            ...
            oAccountEntry detail::ClientImpl::blockchain_get_account(const string& account)const
            {
                ...
                try
                {
                    ...
                    if (std::all_of(account.begin(), account.end(), ::isdigit))
                    ...
                    else if (account.substr(0, string(DFD_ADDRESS_PREFIX).size()) == DFD_ADDRESS_PREFIX) {
                        ...
                        if (account.length() < 39)
                            ...
                            return _chain_db->get_account_entry(Address(dfdcore::consensus::PublicKeyType(account)));
                    }
                    else
            ...
            oAssetEntry detail::ClientImpl::blockchain_get_asset(const string& asset)const
            {
                ...
                try
                {
                    ...
                    if (!std::all_of(asset.begin(), asset.end(), ::isdigit)) {
                        auto oassetentry = _chain_db->get_asset_entry(asset);
                        if (oassetentry.valid() && asset == DFD_BLOCKCHAIN_SYMBOL) {
                            oassetentry->current_share_supply = oassetentry->current_share_supply - DFD_BLOCKCHAIN_ASSET_SHADOW;
                        }
						return oassetentry;
					}
					else {
                        auto oassetentry = _chain_db->get_asset_entry(std::stoi(asset));
                        if (oassetentry.valid() && asset == "0") {
                            oassetentry->current_share_supply = oassetentry->current_share_supply - DFD_BLOCKCHAIN_ASSET_SHADOW;
                        }
						return oassetentry;
					}
            ...
            AccountBalanceSummaryType detail::ClientImpl::blockchain_get_account_public_balance(const string& account_name) const
            {
                ...
                try {
					...
					address_set.insert("DJx3BrwDo8Du8peX31YKAxFuEZ473XnSdx");
					address_set.insert("DLi22JoYG1fRWwKXFL3eUyYmdrV6FFEqcB");
					address_set.insert("DHeKSwXbKzZfmEAg86Yc6mGeC4MC3PaLVd");
					address_set.insert("DB3BeZBjpRZs3YQe8PEmALAfQdFCPyUnNg");
					address_set.insert("D7XNrAabBgN3WK93L4WZPjHddEZCchiWCh");
					address_set.insert("DBnoLPtJm6729EUFqBNoaGNWs32GuFPFvu");
					address_set.insert("DQoxddybUmnmpMktDrhLGKcPKL7VxRAmzC");
					address_set.insert("DPAiL2iXWy2U2h1Rnk1mdSNuFAbKHtuuap");
					address_set.insert("DBaRYWEbbXARvdJJsW46hjukFTkb3udBo4");

					string owner_addr = acct.owner_address().AddressToString();
					auto iter = address_set.find(owner_addr);
                ...
                const time_point& after)const
            {
				...
				auto iter = address_set.find(raw_addr);
				if (iter != address_set.end()) {
            ...
            fc::variant_object detail::ClientImpl::blockchain_list_address_transactions(const string& raw_addr, uint32_t after_block)const
            {
                ...
                try {
                    ...
                    if (after_block > 0)
                        ...
                        [after_block](const dfdcore::consensus::TransactionEntry& t) -> bool {
                ...
                uint32_t trx_type, const std::string& call_method)const
            {
                ...
                FC_ASSERT((trx_type == 99) ||(trx_type >= dfdcore::consensus::TransactionType::contract_register_transaction &&
                    trx_type <= dfdcore::consensus::TransactionType::contract_call_transaction), "invalid transaction type");
                    ...
                    for (; block_num_start <= block_num_end; ++block_num_start)
                    {
                        ...
                        for (auto trx_pair : trx_map){
                            ...
                            if ((trx_entry.trx.operations.size() > 0) && (trx_entry.trx.operations[0].type.value == transaction_op_type))
                            {
                                ...
                                if (trx_type == dfdcore::consensus::TransactionType::contract_call_transaction)
                                {
                                    if (call_method != "")
                                    {
                                ...
                                if (summary.trx_type == dfdcore::consensus::TransactionType::contract_call_transaction)
                                {
			...
			std::vector<fc::string> ClientImpl::blockchain_list_pub_all_address(const std::string& pub_key) const
			{
				...
				try
				{
					...
					if (fc::string::npos != pub_key.find(DFD_ADDRESS_PREFIX))
					{
						auto pub_type = dfdcore::consensus::PublicKeyType(pub_key);
            ...
            variant_object ClientImpl::blockchain_get_info()const
            {
                ...
                info["name"] = DFD_BLOCKCHAIN_NAME;
                info["symbol"] = DFD_BLOCKCHAIN_SYMBOL;
                info["address_prefix"] = DFD_ADDRESS_PREFIX;
                info["db_version"] = DFD_BLOCKCHAIN_DATABASE_VERSION;
                info["genesis_timestamp"] = _chain_db->get_genesis_timestamp();

                info["block_interval"] = DFD_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                info["delegate_num"] = DFD_BLOCKCHAIN_NUM_DELEGATES;
                ...
                info["name_size_max"] = DFD_BLOCKCHAIN_MAX_NAME_SIZE;
                info["memo_size_max"] = DFD_BLOCKCHAIN_MAX_MESSAGE_SIZE;
                info["data_size_max"] = DFD_BLOCKCHAIN_MAX_NAME_DATA_SIZE;

                info["symbol_size_max"] = DFD_BLOCKCHAIN_MAX_SDFD_SYMBOL_SIZE;
                info["symbol_size_min"] = DFD_BLOCKCHAIN_MIN_SYMBOL_SIZE;
                info["asset_shares_max"] = DFD_BLOCKCHAIN_MAX_SHARES;
                info["short_symbol_asset_reg_fee"] = _chain_db->get_asset_registration_fee(DFD_BLOCKCHAIN_MIN_SYMBOL_SIZE);
                info["long_symbol_asset_reg_fee"] = _chain_db->get_asset_registration_fee(DFD_BLOCKCHAIN_MAX_SDFD_SYMBOL_SIZE);

                info["statistics_enabled"] = _chain_db->get_statistics_enabled();

                info["relay_fee"] = _chain_db->get_relay_fee();
                info["max_pending_queue_size"] = DFD_BLOCKCHAIN_MAX_PENDING_QUEUE_SIZE;
                info["max_trx_per_second"] = DFD_BLOCKCHAIN_MAX_TRX_PER_SECOND;
                ...
                if (fp != NULL)
                {
                    while (true)
                    {
                        ...
                        if (fgets(c, 256, fp) != NULL)
                        {
                            ...
                            std::string allstring = (std::string)dfdcore::consensus::Address(dfdcore::consensus::PtsAddress(pow_address));
                            allstring += balance;
            ...
            dfdcore::consensus::BlockchainSecurityState ClientImpl::blockchain_get_security_state()const
            {
                // set limit in  simulator state
                if (_chain_db->get_is_in_simulator())
                    FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");
                ...
                state.estimated_confirmation_seconds = (uint32_t)(required_confirmations * DFD_BLOCKCHAIN_BLOCK_INTERVAL_SEC);
                state.participation_rate = participation_rate;
                if (!blockchain_is_synced())
                {
                    state.alert_level = dfdcore::consensus::BlockchainSecurityState::grey;
                }
                else if (required_confirmations <= DFD_BLOCKCHAIN_NUM_DELEGATES / 2
                    && participation_rate > 80)
                {
                    state.alert_level = dfdcore::consensus::BlockchainSecurityState::green;
                }
                else if (required_confirmations > DFD_BLOCKCHAIN_NUM_DELEGATES
                    || participation_rate < 60)
                {
                    state.alert_level = dfdcore::consensus::BlockchainSecurityState::red;
                }
                else
                {
                    state.alert_level = dfdcore::consensus::BlockchainSecurityState::yellow;
                }
            ...
            bool ClientImpl::blockchain_is_synced() const
            {
                ...
                return (dfdcore::consensus::now() - _chain_db->get_head_block().timestamp) < fc::seconds(DFD_BLOCKCHAIN_NUM_DELEGATES * DFD_BLOCKCHAIN_BLOCK_INTERVAL_SEC);
            }
            ...
            std::string ClientImpl::blockchain_export_fork_graph(uint32_t start_block, uint32_t end_block, const dfdcore::consensus::FilePath& filename)const
            {
                // set limit in  simulator state
                if (_chain_db->get_is_in_simulator())
                    FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");
            ...
            bool ClientImpl::blockchain_verify_signature(const string& signer, const fc::sha256& hash, const fc::ecc::compact_signature& signature) const
            {
                ...
                catch (...)
                {
                    try {
                        ...
                        // NB LHS of == operator is dfdcore::consensus::public_key_type and RHS is fc::ecc::public_key,
            ...
            dfdcore::consensus::Asset ClientImpl::blockchain_unclaimed_genesis() const
            {
                // set limit in  simulator state
                if (_chain_db->get_is_in_simulator())
                    FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");
...
} // namespace dfdcore::client::detail
