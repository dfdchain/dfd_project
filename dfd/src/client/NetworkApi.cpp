...
namespace dfdcore {
    namespace client {
        namespace detail {
            ...
            dfdcore::consensus::TransactionIdType detail::ClientImpl::network_broadcast_transactions(const std::vector<dfdcore::consensus::SignedTransaction>& transactions_to_broadcast)
            {
            ...
            dfdcore::consensus::TransactionIdType detail::ClientImpl::network_broadcast_transaction(const dfdcore::consensus::SignedTransaction& transaction_to_broadcast)
            {
                // set limit in  simulator state
                if (_chain_db->get_is_in_simulator())
                    FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");
            ...
            std::vector<fc::variant_object> detail::ClientImpl::network_get_peer_info(bool hide_firewalled_nodes)const
            {
                ...
                std::vector<dfdcore::network::PeerStatus> peer_statuses = _p2p_node->get_connected_peers();
                for (const dfdcore::network::PeerStatus& peer_status : peer_statuses)
            ...
            // void detail::client_impl::network_set_allowed_peers(const vector<dfdcore::network::node_id_t>& allowed_peers)
            // {
            ...
            dfdcore::network::MessagePropagationData detail::ClientImpl::network_get_transaction_propagation_data(const TransactionIdType& transaction_id)
            {
                // set limit in  simulator state
                if (_chain_db->get_is_in_simulator())
                    FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");
            ...
            dfdcore::network::MessagePropagationData detail::ClientImpl::network_get_block_propagation_data(const BlockIdType& block_id)
            {
                // set limit in  simulator state
                if (_chain_db->get_is_in_simulator())
                    FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");
            ...
            vector<dfdcore::network::PotentialPeerEntry> ClientImpl::network_list_potential_peers()const
            {
