...
using namespace dfdcore::wallet::detail;
    ...
    bool bNeedcreate)
{
    ...
    // 
        ...
        {
            if (store_entry && transaction.dfd_account != "")
            {
                DfdTrxidBalance dfd_entry;
                ...
                for (const auto item : pretty_trx.ledger_entries)
                {
                    ...
                    {
                        dfd_entry.from_account = item.from_account;
                        break;
                    }
                }
                string strToAccount;
                string strSubAccount;
                self->accountsplit(transaction.dfd_account, strToAccount, strSubAccount);
                auto blockchain_trx = _blockchain->get_transaction(transaction_id);
                bool bDiffDeop = false;
                if (blockchain_trx.valid())
                {
                    ShareType dfd_amount = 0;
                    string preowner;
                    int iLoop = 0;
                ...
                //dfd_entry.from_account = transaction.from_account;
                dfd_entry.create_time = transaction_entry->created_time;
                dfd_entry.dfd_account = transaction.dfd_account;
                dfd_entry.block_num = transaction_entry->block_num;
                transaction_entry->trx.dfd_account = transaction.dfd_account;
                transaction_entry->trx.dfd_inport_asset = transaction.dfd_inport_asset;
                dfd_entry.trx_id = transaction.id();
                    //dfd_entry.asset_trx = transaction.dfd_inport_asset;
                    dfd_entry.asset_trx = Asset(dfd_amount, 0);
                    {
                        writelock wlock(m_mutex_for_ub);
                _blockchain->transaction_insert_to_dfd_full_entry(strToAccount, dfd_entry);
                //_blockchain->transaction_insert_to_dfd_balance(dfd_entry.dfd_account, dfd_entry);
                    }
                }
            }
        }
...
bool dfdcore::wallet::detail::WalletImpl::scan_withdraw_contract(const WithdrawContractOperation & op, WalletTransactionEntry & trx_rec, Asset & total_fee, PublicKeyType key, bool bNeedcreate)
{
	try {
...
bool dfdcore::wallet::detail::WalletImpl::scan_ondestroycontract_withdraw(const OnDestroyOperation & op, WalletTransactionEntry & trx_rec, Asset & total_fee, PublicKeyType key, bool bNeedcreate)
{
	try {
		const auto amount = Asset(op.amount); // Always base asset
...
bool dfdcore::wallet::detail::WalletImpl::scan_ondestroycontract_deposit(const OnDestroyOperation & op, WalletTransactionEntry & trx_rec, Asset & total_fee, bool bHaswithdraw)
{
	try {
...
bool dfdcore::wallet::detail::WalletImpl::scan_deposit_contract(const DepositContractOperation & op, WalletTransactionEntry & trx_rec, Asset & total_fee, bool bHaswithdraw)
{
	try{
		auto entry = LedgerEntry();
...
void Wallet::cache_transaction(WalletTransactionEntry& transaction_entry, bool store)
{
    try {
		...
		if (store)
		{
			...
			transaction_entry.created_time = dfdcore::consensus::now();
    ...
    TrxType trx_search_type)const
{
    try {
        ...
        if (!asset_symbol.empty() && asset_symbol != DFD_BLOCKCHAIN_SYMBOL)
        {
            try
            {
        ...
        for (const auto& item : transactions)
        {
            ...
            if (!tx_entry.is_confirmed && (tx_entry.trx.expiration < dfdcore::consensus::now()))
            {
                continue;
            }
...
PrettyTransaction		Wallet::to_pretty_trx(const dfdcore::consensus::TransactionEntry& trx_entry, const std::string addr_for_fee) const
{
    ...
    pretty_trx.trx_type = dfdcore::consensus::TransactionType::normal_transaction;
    ...
    for (const auto & op : trx_entry.trx.operations)
    {
		...
		case withdraw_pay_op_type:
		{
            ...
            pretty_trx.trx_type = dfdcore::consensus::TransactionType::withdraw_pay_transaction;
			break;
		}
    ...
    for (const auto& op : trx.operations)
    {
        ...
        case register_account_op_type:
        {
            ...
            if (register_op.delegate_pay_rate > 100)
            {
                ...
                pretty_trx.trx_type = dfdcore::consensus::TransactionType::register_account_transaction;
            }
            else
            {
                pretty_entry.memo = "register delegate " + account_name_entry->name; // Can't tell if initially registered as a delegate
                pretty_trx.trx_type = dfdcore::consensus::TransactionType::register_delegate_transaction;
            }
		...
		case update_account_op_type:
		{
            ...
            //所以现在只能简单粗暴得用一个1000DFD来进行划分，只要是大于1000DFD都认为是升级代理
            if (total_fee <= Asset( 1000 * DFD_BLOCKCHAIN_PRECISION ))
            {
                pretty_entry.memo = "update " + account_name_entry->name;
                pretty_trx.trx_type = dfdcore::consensus::TransactionType::update_account_transaction;
            }
            else
            {
                pretty_entry.memo = "upgrade " + account_name_entry->name;
                pretty_trx.trx_type = dfdcore::consensus::TransactionType::upgrade_account_transaction;
            }
...
PrettyContractTransaction		Wallet::to_pretty_contract_trx(const dfdcore::consensus::TransactionEntry& trx_entry) const
{
    PrettyContractTransaction pretty_trx;
    SignedTransaction result_trx = trx_entry.trx;

    FC_ASSERT((trx_entry.trx.operations.size() > 0) && (trx_entry.trx.operations[0].type == dfdcore::consensus::OperationTypeEnum::transaction_op_type) , 
        "not result contract transaction");
    SignedTransaction orig_trx = trx_entry.trx.operations[0].as<dfdcore::consensus::TransactionOperation>().trx;
    ...
    FC_ASSERT((orig_trx.operations.size() > 0) && 
        ((orig_trx.operations[0].type >= fc::enum_type<uint8_t, dfdcore::consensus::OperationTypeEnum>(contract_register_op_type))
            && (orig_trx.operations[0].type <= fc::enum_type<uint8_t, dfdcore::consensus::OperationTypeEnum>(transfer_contract_op_type))), "invalid original contract transaction");


    dfdcore::consensus::OperationTypeEnum contract_op_type = orig_trx.operations[0].type;
    ...
    if (contract_op_type == dfdcore::consensus::OperationTypeEnum::contract_register_op_type)
    {
        pretty_trx.trx_type = dfdcore::consensus::TransactionType::contract_register_transaction;

        PrettyContractLedgerEntry to_contract_ledger_entry;
        vector<PrettyContractLedgerEntry> from_contract_ledger_entries;

        dfdcore::consensus::RegisterContractOperation contract_register_op = orig_trx.operations[0].as<dfdcore::consensus::RegisterContractOperation>();
        Address contract_owner = dfdcore::consensus::Address(contract_register_op.owner);
        ...
        to_contract_ledger_entry.amount = Asset(DFD_DEFAULT_CONTRACT_MARGIN, 0);
        ...
        {
            // 注册的合约成功上链
            if (op.type == fc::enum_type<uint8_t, dfdcore::consensus::OperationTypeEnum>(contract_info_op_type))
                register_success = true;
            
            if (op.type == fc::enum_type<uint8_t, dfdcore::consensus::OperationTypeEnum>(balances_withdraw_op_type))
            {
        ...
        if (register_success)
        {
            to_contract_ledger_entry.fee = Asset(all_cost - DFD_DEFAULT_CONTRACT_MARGIN, 0);
    ...
    else if (contract_op_type == dfdcore::consensus::OperationTypeEnum::contract_upgrade_op_type)
    {
        pretty_trx.trx_type = dfdcore::consensus::TransactionType::contract_upgrade_transaction;

        PrettyContractLedgerEntry to_contract_ledger_entry;
        vector<PrettyContractLedgerEntry> from_contract_ledger_entries;

        dfdcore::consensus::UpgradeContractOperation contract_upgrade_op = orig_trx.operations[0].as<dfdcore::consensus::UpgradeContractOperation>();
        ...
        {
            // 合约成功升级
            if (op.type == fc::enum_type<uint8_t, dfdcore::consensus::OperationTypeEnum>(on_upgrade_op_type))
            ...
            // 合约出账(含保证金) 
            if (op.type == fc::enum_type<uint8_t, dfdcore::consensus::OperationTypeEnum>(withdraw_contract_op_type))
            {
            ...
            // 用户入账
            if (op.type == fc::enum_type<uint8_t, dfdcore::consensus::OperationTypeEnum>(deposit_op_type))
            {
    ...
    else if (contract_op_type == dfdcore::consensus::OperationTypeEnum::contract_destroy_op_type)
    {
        pretty_trx.trx_type = dfdcore::consensus::TransactionType::contract_destroy_transaction;

        PrettyContractLedgerEntry to_contract_ledger_entry;
        vector<PrettyContractLedgerEntry> from_contract_ledger_entries;

        dfdcore::consensus::DestroyContractOperation contract_destroy_op = orig_trx.operations[0].as<dfdcore::consensus::DestroyContractOperation>();
        ...
        {
            // 合约成功销毁
            if (op.type == fc::enum_type<uint8_t, dfdcore::consensus::OperationTypeEnum>(on_destroy_op_type))
            {
    ...
    else if (contract_op_type == dfdcore::consensus::OperationTypeEnum::contract_call_op_type)
    {
        pretty_trx.trx_type = dfdcore::consensus::TransactionType::contract_call_transaction;

        PrettyContractLedgerEntry to_contract_ledger_entry;
        vector<PrettyContractLedgerEntry> from_contract_ledger_entries;

        dfdcore::consensus::CallContractOperation contract_call_op = orig_trx.operations[0].as<dfdcore::consensus::CallContractOperation>();
        Address contract_caller = dfdcore::consensus::Address(contract_call_op.caller);
        ...
        {
            // 合约成功调用
            if (op.type == fc::enum_type<uint8_t, dfdcore::consensus::OperationTypeEnum>(on_call_success_op_type))
                call_success = true;
    ...
    else if (contract_op_type == dfdcore::consensus::OperationTypeEnum::transfer_contract_op_type)
    {
        pretty_trx.trx_type = dfdcore::consensus::TransactionType::transfer_contract_transaction;

        PrettyContractLedgerEntry to_contract_ledger_entry;
        vector<PrettyContractLedgerEntry> from_contract_ledger_entries;

        dfdcore::consensus::TransferContractOperation transfer_contract_op = orig_trx.operations[0].as<dfdcore::consensus::TransferContractOperation>();
        Address contract_depositer = dfdcore::consensus::Address(transfer_contract_op.from);
        ...
        {
            // 向合约成功充值
            if (op.type == fc::enum_type<uint8_t, dfdcore::consensus::OperationTypeEnum>(deposit_contract_op_type))
