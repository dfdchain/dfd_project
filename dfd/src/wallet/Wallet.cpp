...
namespace dfdcore {
    namespace wallet {
        ...
        namespace detail {
            ...
            {
                if (!self->is_open() || !self->is_unlocked()) return;
                auto ntptime = dfdcore::consensus::ntp_time();
				fc::time_point t = (ntptime.valid() ? *ntp_time() : fc::time_point::now());
                if ((t - _blockchain->get_head_block_timestamp()) <= fc::seconds(DFD_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 2))
            ...
            */
                ...
                try {
                    ...
                    try
                    {
                        ...
                        const auto update_progress = [=](const uint32_t count)
                        {
                            ...
                            if (count % 10000 == 0) ulog("Scanning ${p} done...", ("p", dfdcore::console::pretty_percent(_scan_progress, 1)));
                        };
            ...
            void WalletImpl::upgrade_version()
            {
                ...
                if (current_version > DFD_WALLET_VERSION)
                {
                    FC_THROW_EXCEPTION(unsupported_version, "Wallet version newer than client supports!",
                        ("wallet_version", current_version)("supported_version", DFD_WALLET_VERSION));
                }
                else if (current_version == DFD_WALLET_VERSION)
                {
                    return;
                }
                ...
                try
                {
                    ...
                    {
                        self->set_transaction_fee(Asset(DFD_WALLET_DEFAULT_TRANSACTION_FEE));
                        self->set_transaction_imessage_fee_coe(DFD_BLOCKCHAIN_MIN_MESSAGE_FEE_COE);
                        self->set_transaction_imessage_soft_max_length(DFD_BLOCKCHAIN_MAX_SOFT_MAX_MESSAGE_SIZE);
                    }

                    if (current_version < 106)
                    {
                        self->set_transaction_expiration(DFD_WALLET_DEFAULT_TRANSACTION_EXPIRATION_SEC);
                    }
                    ...
                    {
                        self->set_version(DFD_WALLET_VERSION);
                ...
                const string& brainkey)
            {
                try {
                    ...
                    if (password.size() < DFD_WALLET_MIN_PASSWORD_LENGTH)
             ...
             *  returned will contain no more than DFD_BLOCKCHAIN_MAX_SLATE_SIZE delegates.
             */
            SlateEntry WalletImpl::get_delegate_slate(const VoteStrategy strategy)const
            {
                ...
                {
                    slate_size = std::min<size_t>(DFD_BLOCKCHAIN_MAX_SLATE_SIZE, for_candidates.size());
                }
                else if (strategy == vote_random)
                {
                    slate_size = std::min<size_t>(DFD_BLOCKCHAIN_MAX_SLATE_SIZE / 3, for_candidates.size());
                    slate_size = rand() % (slate_size + 1);
                }
                else if (strategy == vote_recommended && for_candidates.size() < DFD_BLOCKCHAIN_MAX_SLATE_SIZE)
                {
                    ...
                    while (for_candidates.size() < DFD_BLOCKCHAIN_MAX_SLATE_SIZE && recommended_candidate_ranks.size() > 0)
                    {
                        int best_rank = 0;
                ...
                FC_ASSERT(entry.slate.size() <= DFD_BLOCKCHAIN_MAX_SLATE_SIZE);
            ...
            const string& brainkey)
        {
            try {
                ...
                try
                {
                    ...
                    open(wallet_name);
                    unlock(password, DFD_WALLET_DEFAULT_UNLOCK_TIME_SEC);
                }
                catch (...)
                {
        ...
        dfdcore::wallet::WalletTransactionEntry Wallet::transfer_asset_to_contract_without_signature(
            ...
            bool is_testing)
        {
            try {
                ...
                FC_ASSERT(is_open(), "Wallet not open!");
                FC_ASSERT(amount_to_transfer_symbol == DFD_BLOCKCHAIN_SYMBOL, "Asset symbol should be HSR");
                ...
                FC_ASSERT(asset_id == 0, "Asset symbol should be DFD");
                ...
                Asset asset_to_transfer = my->_blockchain->to_ugly_asset(real_amount_to_transfer, DFD_BLOCKCHAIN_SYMBOL);
            ...
            const string& dfd_account)
        {
            try {
                ...
                if (dfd_account != "")
                {
                    ...
                    trx.dfd_account = dfd_account;
                    trx.dfd_inport_asset = asset_to_transfer;
                }
                // 	  if (memo_message != "")
                // 	  {
        ...
        void Wallet::create_from_json(const path& filename, const string& wallet_name, const string& passphrase)
        {
            try {
                ...
                try
                {
                    ...
                    close();
                    open(wallet_name);
                    unlock(passphrase, DFD_WALLET_DEFAULT_UNLOCK_TIME_SEC);
                }
                catch (...)
                {
        ...
        void Wallet::unlock(const string& password, uint32_t timeout_seconds)
        {
            try {
                ...
                try
                {
                    ...
                    //const uint32_t max_timeout_seconds = std::numeric_limits<uint32_t>::max() - dfdcore::consensus::now().sec_since_epoch();
                    const uint32_t max_timeout_seconds = std::numeric_limits<int32_t>::max() - dfdcore::consensus::now().sec_since_epoch();
        ...
        void Wallet::lock()
        {
            ...
            ilog("Wallet locked at time: ${t}", ("t", dfdcore::consensus::now()));
        ...
        bool Wallet::check_passphrase(const string& passphrase)
        {
            try {
                ...
                if (passphrase.size() < DFD_WALLET_MIN_PASSWORD_LENGTH) FC_CAPTURE_AND_THROW(password_too_short);
        ...
        void Wallet::change_passphrase(const string & old_passphrase, const string& new_passphrase)
        {
            try {
                if (NOT is_open()) FC_CAPTURE_AND_THROW(wallet_closed);
                if (NOT is_unlocked()) FC_CAPTURE_AND_THROW(wallet_locked);
                if (new_passphrase.size() < DFD_WALLET_MIN_PASSWORD_LENGTH) FC_CAPTURE_AND_THROW(password_too_short);
            ...
            const variant& private_data)
        {
            try {
                ...
                if (current_account.valid())
                {
                    ...
                    {
                        dfdcore::consensus::AccountEntry& as_blockchain_account_entry = *current_account;
                ...
                else
                {
                    ...
                    if (!current_registered_account.valid())
                    {
                        const time_point_sec now = dfdcore::consensus::now();
        ...
        void Wallet::accountsplit(const string & original, string & to_account, string & sub_account)
        {
            ...
            if (INVALIDE_SDFD_ADDRESS == sub_account)
            {
                sub_account = "";
            }
            else
            ...
            bool create_account)
        {
            try {
                ...
                if (blockchain_account_entry.valid())
                {
                    ...
                    account_entry->set_active_key(dfdcore::consensus::now(), active_public_key);
                    account_entry->last_update = dfdcore::consensus::now();
            ...
            bool create_account)
        {
            try {
                ...
                auto key = dfdcore::utilities::wif_to_key(wif_key);
                if (key.valid())
                {
        ...
        VoteSummary Wallet::get_vote_status(const string& account_name)
        {
            ...
            for (const auto& item : items)
            {
                ...
                for (const auto& entry : entrys)
                {
                    if (entry.asset_id() != 0)
                        continue;
                    if (entry.slate_id() != my_slate.id() && entry.balance > 1 * DFD_BLOCKCHAIN_PRECISION) // ignore dust
                    ...
                    total_possible += entry.get_spendable_balance(my->_blockchain->now()).amount * DFD_BLOCKCHAIN_MAX_SLATE_SIZE;
                }
            }
            ...
            bool sign)
        {
            try {
                ...
                trx.expiration = dfdcore::consensus::now() + get_transaction_expiration();
            ...
            bool sign)
        {
            try {
                ...
                const auto version = dfdcore::client::version_info()["client_version"].as_string();
        ...
        void Wallet::repair_entrys(const optional<string>& collecting_account_name)
        {
            try {
                ...
                for (const auto& item : items)
                {
                    ...
                    if (name.find(DFD_ADDRESS_PREFIX) != 0)
                        continue;
        ...
        std::map<std::string, dfdcore::consensus::DelegatePaySalary> Wallet::query_delegate_salarys()
        {
            FC_ASSERT(is_open());
            FC_ASSERT(is_unlocked());
            std::map<std::string, dfdcore::consensus::DelegatePaySalary> Salary_Map;
        ...
        WalletTransactionEntry Wallet::contract_register(const string& owner, const fc::path codefile, const string& asset_symbol, double init_limit, bool is_testing)
        {
            ...
            {
                FC_THROW_EXCEPTION(dfdcore::consensus::invalid_contract_filename, "contract bytecode file name should end with .gpc");
            }
            ...
            FC_ASSERT(asset_symbol == DFD_BLOCKCHAIN_SYMBOL, "asset_symbol must be DFD");

            const auto asset_rec = data_ptr->get_asset_entry(asset_symbol);
            FC_ASSERT(asset_rec.valid(), "Asset not exist!");
            FC_ASSERT(asset_rec->id == 0, "asset_symbol must be DFD");
            ...
            FC_ASSERT(register_fee.asset_id == 0, "register fee must be DFD");
            FC_ASSERT(margin.asset_id == 0, "register fee must be DFD");
            FC_ASSERT(fee.asset_id == 0, "register fee must be DFD");
            trx.expiration = dfdcore::consensus::now() + get_transaction_expiration();
            my->sign_transaction(trx, required_signatures);
        ...
        std::vector<dfdcore::consensus::Asset> Wallet::contract_register_testing(const string& owner, const fc::path codefile)
        {
            FC_ASSERT(is_open(), "Wallet not open!");
            FC_ASSERT(is_unlocked(), "Wallet not unlock!");
            FC_ASSERT(my->is_receive_account(owner), "Invalid account name");
            ...
            auto trans_entry = contract_register(owner,
                codefile,
                DFD_BLOCKCHAIN_SYMBOL,
                (double)asset_for_exec.amount / DFD_BLOCKCHAIN_PRECISION, 
                true
                );
            ...
            std::vector<dfdcore::consensus::Asset> asset_vec;
		...
		dfdcore::wallet::WalletTransactionEntry Wallet::contract_call(const string caller, const ContractIdType contract, const string method, const string& arguments, const string& asset_symbol, double cost_limit, bool is_testing)
		{
			ChainInterfacePtr data_ptr = get_correct_state_ptr();
        ...
        std::vector<dfdcore::consensus::Asset> Wallet::contract_call_testing(const string caller, const ContractIdType contract, const string method, const string& arguments)
        {
            FC_ASSERT(is_open(), "Wallet not open!");
            FC_ASSERT(is_unlocked(), "Wallet not unlock!");
            FC_ASSERT(my->is_receive_account(caller), "Invalid account name");
        ...
        std::string Wallet::contract_call_offline(const string caller, const ContractIdType contract, const string method, const string& arguments)
        {
            ...
            if (exception_code > 0)
            {
                dfdcore::consensus::contract_error con_err(32000, "exception", exception_msg);
                throw con_err;
            }
        ...
        dfdcore::wallet::oScriptEntry Wallet::get_script_entry(const ScriptIdType& script_id)
        {
            try{
                FC_ASSERT(is_open(), "Wallet not open!");
                FC_ASSERT(is_unlocked(), "Wallet not unlock!");
        ...
        dfdcore::consensus::ScriptIdType Wallet::script_add(const fc::path& filename, const string& description/*=string("")*/)
        {
            try{
                FC_ASSERT(is_open(), "Wallet not open!");
                FC_ASSERT(is_unlocked(), "Wallet not unlock!");
        ...
        dfdcore::wallet::WalletTransactionEntry Wallet::transfer_asset_to_contract(
            const string& real_amount_to_transfer,
            const string& amount_to_transfer_symbol,
            const string& from_account_name,
            const Address& to_contract_address,
            ...
            bool is_testing)
        {
            try {
                ...
                FC_ASSERT(is_open(), "Wallet not open!");
                FC_ASSERT(is_unlocked(), "Wallet not unlock!");
                FC_ASSERT(amount_to_transfer_symbol == DFD_BLOCKCHAIN_SYMBOL, "Asset symbol should be DFD");
                FC_ASSERT(data_ptr->is_valid_symbol(amount_to_transfer_symbol), "Invalid asset symbol");
                FC_ASSERT(my->is_receive_account(from_account_name), "Invalid account name");
        ...
        std::vector<dfdcore::consensus::Asset> Wallet::transfer_asset_to_contract_testing(
            const string& real_amount_to_transfer,
            const string& amount_to_transfer_symbol,
            const string& from_account_name,
            const Address& to_contract_address,
		...
		std::vector<dfdcore::consensus::Asset> Wallet::contract_upgrade_testing(
			...
			)
		{
			try
			{
				...
				if (margin_balance_entry->balance != DFD_DEFAULT_CONTRACT_MARGIN)
					FC_CAPTURE_AND_THROW(invalid_margin_amount, (margin_balance_entry->balance));


				Asset margin = Asset(DFD_DEFAULT_CONTRACT_MARGIN);
				Asset fee = get_transaction_fee(0);
				Asset asset_for_exec = my->_blockchain->get_amount(CONTRACT_TESTING_LIMIT_MAX);
			...
			bool is_testing)
        {
            try
			{
				...
				FC_ASSERT(balance_entry->asset_id() == 0, "margin balance asset is not DFD!");
		...
		std::vector<dfdcore::consensus::Asset> Wallet::contract_destroy_testing(
			const Address& contract_id,
			...
			bool is_testing)
        {
            try
			{
                ...
                FC_ASSERT(balance_entry.valid(), "Can not get margin balance!");
                FC_ASSERT(balance_entry->asset_id() == 0, "margin balance asset is not DFD!");
                FC_ASSERT(balance_entry->balance == DFD_DEFAULT_CONTRACT_MARGIN, "invalid margin balance amount");
                ...
                {
                    FC_ASSERT(balance_entry->asset_id() == 0, "contract balance asset is not DFD!");
                }
            ...
            bool sign)
        {
            try {
                ...
                {
                    required_fees += Asset(DFD_BLOCKCHAIN_REGISTER_ACCOUNT_FEE, 0);
                }
        ...
        {
            try {
                FC_ASSERT(dfdcore::consensus::is_power_of_ten(precision));
                FC_ASSERT(is_open(), "Wallet not open!");
                FC_ASSERT(is_unlocked(), "Wallet not unlock!");
                ...
                FC_ASSERT(DFD_BLOCKCHAIN_MAX_SHARES > max_share_supply_in_internal_units);
        ...
        void Wallet::set_transaction_fee(const Asset& fee)
        {
            try {
                ...
                if (fee.amount < DFD_DEFAULT_TRANSACTION_FEE || fee.asset_id != 0)
        ...
        Asset Wallet::get_transaction_imessage_fee(const std::string & imessage)const
        {
            try
            {
                ...
                Asset require_fee;
                    ...
                    if (DFD_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE >= imessage.size())
                    {
                        return require_fee;
                    }
                    auto min_fee_coe = get_transaction_imessage_fee_coe();
                    require_fee.amount = min_fee_coe * (imessage.size() - DFD_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE);
        ...
        Asset Wallet::get_transaction_fee(const AssetIdType desired_fee_asset_id)const
        {
            try {
                ...
                Asset xts_fee(DFD_WALLET_DEFAULT_TRANSACTION_FEE, 0);
                try
                {
        ...
        void Wallet::set_last_scanned_block_number_for_ub(uint32_t block_num)
        {
            try {
                FC_ASSERT(is_open(), "Wallet not open!");
                my->_wallet_db.set_property(last_scanned_block_number_for_dfdcore, fc::variant(block_num));
            } FC_CAPTURE_AND_RETHROW()
        }
        ...
        uint32_t Wallet::get_last_scanned_block_number_for_ub()const
        {
            try {
                ...
                try
                {
                    return my->_wallet_db.get_property(last_scanned_block_number_for_dfdcore).as<uint32_t>();
                }
                catch (...)
                {
                }
        ...
        void Wallet::set_transaction_imessage_fee_coe(const ImessageIdType& coe)
        {
            try
            {
                FC_ASSERT(is_open(), "Wallet not open!");
                    ...
                    if (coe < DFD_BLOCKCHAIN_MIN_MESSAGE_FEE_COE)
                    {
        ...
        void Wallet::set_transaction_imessage_soft_max_length(const ImessageLengthIdType& length)
        {
            try
            {
                FC_ASSERT(is_open(), "Wallet not open!");
                    ...
                    if (length < 0 || length > DFD_BLOCKCHAIN_MAX_MESSAGE_SIZE)
                    {
        ...
        void Wallet::set_transaction_expiration(uint32_t secs)
        {
            try {
                ...
                if (secs > DFD_BLOCKCHAIN_MAX_TRANSACTION_EXPIRATION_SEC)
