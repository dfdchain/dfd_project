...
void  TransactionBuilder::set_wallet_implementation(std::unique_ptr<dfdcore::wallet::detail::WalletImpl>& wimpl)
{
    _wimpl = wimpl.get();
}
    ...
    ShareType     amount_to_receiver)
{
    try {
        ...
        if (trx.expiration == time_point_sec())
            trx.expiration = dfdcore::consensus::now() + _wimpl->self->get_transaction_expiration();
...
TransactionBuilder& TransactionBuilder::deposit_asset(const dfdcore::wallet::WalletAccountEntry& payer,
    const dfdcore::consensus::AccountEntry& recipient,
    const Asset& amount,
    const string& memo,
    ...
    const string& dfd_account)
{
    try {
        ...
        if (memo.size() > DFD_BLOCKCHAIN_MAX_EXTENDED_MEMO_SIZE)
...
TransactionBuilder& TransactionBuilder::deposit_asset_with_escrow(const dfdcore::wallet::WalletAccountEntry& payer,
    const dfdcore::consensus::AccountEntry& recipient,
    const dfdcore::consensus::AccountEntry& escrow_agent,
    ...
    const string& dfd_account)
{
    try {
        ...
        // Don't automatically truncate memos as long as users still depend on them via deposit ops rather than mail
        if (memo.size() > DFD_BLOCKCHAIN_MAX_MEMO_SIZE)
            FC_CAPTURE_AND_THROW(memo_too_long, (memo));
    ...
    )
{
    try {
        ...
        if (market_fee >= 0 || market_fee <= DFD_BLOCKCHAIN_MAX_UIA_MARKET_FEE)
            actual_market_fee = DFD_BLOCKCHAIN_MAX_UIA_MARKET_FEE * market_fee;
