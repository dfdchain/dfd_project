...
namespace dfdcore {
    namespace rpc {
        ...
        namespace detail
        {
            class RpcClientImpl : public dfdcore::rpc_stubs::CommonApiRpcClient
            {
            public:
                ...
                void connect_to(const fc::ip::endpoint& remote_endpoint, const dfdcore::consensus::PublicKeyType& remote_public_key);
                bool login(const std::string& username, const std::string& password);
            };

            void RpcClientImpl::connect_to(const fc::ip::endpoint& remote_endpoint,
                const dfdcore::consensus::PublicKeyType& remote_public_key)
            {
                fc::buffered_istream_ptr buffered_istream;
                fc::buffered_ostream_ptr buffered_ostream;

                if (remote_public_key != dfdcore::consensus::PublicKeyType())
                {
                    dfdcore::network::StcpSocketPtr socket = std::make_shared<dfdcore::network::StcpSocket>();
                    ...
                    wdump((signature));
                    FC_ASSERT(dfdcore::consensus::PublicKeyType(fc::ecc::public_key(signature, fc::digest(socket->get_shared_secret())))
                        == remote_public_key,
