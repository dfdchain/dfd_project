...
namespace dfdcore {
    namespace rpc {
        ...
        namespace detail
        {
            class RpcServerImpl : public dfdcore::rpc_stubs::CommonApiRpcServer
            {
            public:
                RpcServerConfig                                 _config;
                dfdcore::client::Client*                              _client;
                ...
                typedef std::map<std::string, dfdcore::api::MethodData> MethodMapType;
                ...
                RpcServerImpl(dfdcore::client::Client* client) :
                    _client(client),
                ...
                virtual dfdcore::api::CommonApi* get_client() const override;
                ...
                virtual void store_method_metadata(const dfdcore::api::MethodData& method_metadata);

                std::string help(const std::string& command_name) const;

                std::string make_short_description(const dfdcore::api::MethodData& method_data, bool show_decription = true) const
                {
                    std::string help_string;
                    std::stringstream short_description;
                    short_description << std::setw(100) << std::left;
                    help_string = method_data.name + " ";
                    for (const dfdcore::api::ParameterData& parameter : method_data.parameters)
                    {
                        if (parameter.classification == dfdcore::api::required_positional)
                            help_string += std::string("<") + parameter.name + std::string("> ");
                        else if (parameter.classification == dfdcore::api::optional_named)
                            help_string += std::string("{") + parameter.name + std::string("} ");
                        else if (parameter.classification == dfdcore::api::required_positional_hidden)
                            continue;
                        else
                            help_string += std::string("[") + parameter.name + std::string("] ");
                    }
                    ...
                    try {
                        if (_config.rpc_user.size())
                        {
                                ...
                                _config.rpc_password != password)
                            {
                                ...
                                s.add_header("WWW-Authenticate", "Basic realm=\"dfdcore wallet\"");
                ...
                fc::http::reply::status_code handle_http_rpc(const fc::http::request& r, const fc::http::server::response& s)
                {
                    ...
                    try {
                        ...
                        if (fc::time_point(dfdcore::consensus::now()) - _last_cache_clear_time > fc::seconds(DFD_BLOCKCHAIN_BLOCK_INTERVAL_SEC))
                        {
                            _last_cache_clear_time = dfdcore::consensus::now();
                            _call_cache.clear();
                        }
                ...
                void encrypted_accept_loop(const dfdcore::consensus::PrivateKeyType& server_key)
                {
                    while (!_encrypted_accept_loop_complete.canceled())
                    {
                        dfdcore::network::StcpSocketPtr sock = std::make_shared<dfdcore::network::StcpSocket>();
                        try
                        {
                ...
                fc::variant dispatch_method_from_json_connection(fc::rpc::json_connection* con,
                    const dfdcore::api::MethodData& method_data,
                    const fc::variants& arguments)
                {
                    // ilog( "arguments: ${params}", ("params",arguments) );
                    if ((method_data.prerequisites & dfdcore::api::json_authenticated) &&
                ...
                fc::variant dispatch_authenticated_method(const dfdcore::api::MethodData& method_data,
                    const fc::variants& arguments_from_caller)
                {
                    ...
                    if (method_data.prerequisites & dfdcore::api::wallet_open)
                        verify_wallet_is_open();
                    if (method_data.prerequisites & dfdcore::api::wallet_unlocked)
                        verify_wallet_is_unlocked();
                    if (method_data.prerequisites & dfdcore::api::connected_to_network)
                        verify_connected_to_network();
                    if (method_data.prerequisites & dfdcore::api::simulator_open)
                    ...
                    {
                        if (parameter.classification == dfdcore::api::required_positional
                            || parameter.classification == dfdcore::api::required_positional_hidden)
                        {
                        ...
                        else if (parameter.classification == dfdcore::api::optional_positional)
                        {
            ...
            dfdcore::api::CommonApi* RpcServerImpl::get_client() const
            {
                return _client;
            }
            ...
            void RpcServerImpl::store_method_metadata(const dfdcore::api::MethodData& method_metadata)
            {
            ...
            std::string RpcServerImpl::help(const std::string& command_name) const
            {
                ...
                { //display detailed description of requested command
                    ...
                    if (alias_iter != _alias_map.end())
                    {
                        ...
                        const dfdcore::api::MethodData& method_data = method_iter->second;
                        help_string += "Usage:\n";
        ...
        RpcServer::RpcServer(dfdcore::client::Client* client) :
        ...
        const dfdcore::api::MethodData& RpcServer::get_method_data(const std::string& method_name)
        {
        ...
        std::vector<dfdcore::api::MethodData> RpcServer::get_all_method_data() const
        {
            std::vector<dfdcore::api::MethodData> result;
            for (const detail::RpcServerImpl::MethodMapType::value_type& value : my->_method_map)
                result.emplace_back(value.second);
            return result;
        }

        void RpcServer::register_method(dfdcore::api::MethodData data)
        {
