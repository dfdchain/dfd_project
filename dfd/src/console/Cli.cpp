...
namespace dfdcore {
    namespace console {
        ...
        FC_DECLARE_EXCEPTION(cli_exception, 11000, "CLI Error")
            FC_DECLARE_DERIVED_EXCEPTION(abort_cli_command, dfdcore::console::cli_exception, 11001, "command aborted by user");
        FC_DECLARE_DERIVED_EXCEPTION(exit_cli_command, dfdcore::console::cli_exception, 11002, "exit CLI client requested by user");
            ...
            )
        {
            if (input_stream == nullptr)
                FC_CAPTURE_AND_THROW(dfdcore::console::exit_cli_command); //_input_stream != nullptr );
        ...
        namespace detail
        {
            ...
            public:
                ...
                CliImpl(dfdcore::client::Client* client, std::istream* command_script, std::ostream* output_stream);
                    ...
                    fc::istream_ptr argument_stream)
                {
                    ...
                    catch (const dfdcore::console::abort_cli_command&)
                    {
                        throw;
                    }
                    catch (const fc::exception& e)
                    ...
                    if (command_is_valid)
                    {
                        ...
                        catch (const dfdcore::console::exit_cli_command&)
                        {
                            throw;
                        }
                        catch (const fc::exception& e)
                ...
                string get_line(const string& prompt = CLI_PROMPT_SUFFIX, bool no_echo = false)
                {
                    ...
                    return dfdcore::console::get_line(
                        _input_stream,
                        _out,
                        prompt,
                        no_echo,
                ...
                fc::variants parse_interactive_command(fc::buffered_istream& argument_stream, const string& command)
                {
                    try
                    {
                        const dfdcore::api::MethodData& method_data = _rpc_server->get_method_data(command);
                    ...
                    const dfdcore::api::MethodData& method_data)
                {
                    try {
                        ...
                        for (unsigned i = 0; i < method_data.parameters.size(); ++i)
                        {
                            ...
                            {
                                if (method_data.parameters[i].classification != dfdcore::api::required_positional)
                                {
                                    return arguments;
                                }
                                else //if missing required argument, prompt for that argument
                                {
                                    const dfdcore::api::ParameterData& this_parameter = method_data.parameters[i];
                            ...
                            if (method_data.parameters[i].classification == dfdcore::api::optional_named)
                                break;
                        }
                        return arguments;
                ...
                {
                    try {
                        const dfdcore::api::ParameterData& this_parameter = method_data.parameters[parameter_index];
                        if (this_parameter.type == "asset")
                        {
                            ...
                            return fc::variant(dfdcore::consensus::Asset(amount_as_int));
                        }
                        else if (this_parameter.type == "address")
                        {
                            ...
                            if (!dfdcore::consensus::Address::is_valid(address_string))
                            {
                            ...
                            return fc::variant(dfdcore::consensus::Address(address_string));
                        }
...
#ifdef WIN32
							string encodeline;
							if (dfdcore::utilities::isGBK(result.data()))
							{
                ...
                fc::variant execute_interactive_command(const string& command, const fc::variants& arguments)
                {
                    ...
                    {
                        _quit = true;
                        dfdcore::client::g_client_quit = true;
                        FC_THROW_EXCEPTION(dfdcore::console::exit_cli_command, "quit command issued");
                    }
						...
						if (_client->get_chain()->get_is_in_simulator())
						{
							...
							if (pos == cmd_limit.end())
							{
								*_out << "In simulator you can not execute this command! \n";
								...
								FC_THROW_EXCEPTION(dfdcore::console::abort_cli_command, "");
							}
                ...
                string prompt_for_input(const string& prompt, bool no_echo = false, bool verify = false)
                {
                    try {
                        ...
                        while (true)
                        {
                            ...
                            if (input.empty())
                                FC_THROW_EXCEPTION(dfdcore::console::abort_cli_command, "input aborted");
                    ...
                    const fc::variant& result)
                {
                    ...
                    {
                        string result_type;
                        const dfdcore::api::MethodData& method_data = _rpc_server->get_method_data(method_name);
                        result_type = method_data.return_type;

                        if (result_type == "asset")
                        {
                            *_out << (string)result.as<dfdcore::consensus::Asset>() << "\n";
                        }
                        else if (result_type == "address")
                        {
                            *_out << (string)result.as<dfdcore::consensus::Address>() << "\n";
                        }
...
#ifdef HAVE_READLINE
                typedef std::map<string, dfdcore::api::MethodData> MethodDataMapType;
            ...
            CliImpl::CliImpl(dfdcore::client::Client* client, std::istream* command_script, std::ostream* output_stream)
                :_client(client)
      ...
      //rl_bind_keyseq("\\C-c", &control_c_handler);
            ...
            void CliImpl::initialize_method_data_if_necessary()
            {
                if (!_method_data_is_initialized)
                {
                    ...
                    vector<dfdcore::api::MethodData> method_data_list = _rpc_server->get_all_method_data();
            ...
            char** CliImpl::json_completion(const char* text, int start, int end)
            {
                ...
                else
                {
                    ...
                    if (!trimmed_command_to_parse.empty())
                    {
                        ...
                        try
                        {
                            const dfdcore::api::MethodData& method_data = cli_impl_instance->_rpc_server->get_method_data(trimmed_command_to_parse);
                            if (method_data.name == "help")
                            {
                                return rl_completion_matches(text, &json_command_completion_generator_function);
                            }
                        }
                        catch (const dfdcore::rpc::unknown_method&)
                        {
                            // do nothing
                        }
                    }
        ...
        Cli::Cli(dfdcore::client::Client* client, std::istream* command_script, std::ostream* output_stream)
