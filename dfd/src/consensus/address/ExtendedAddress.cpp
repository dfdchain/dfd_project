    ...
    namespace consensus {
        ...
        ExtendedAddress::ExtendedAddress(const std::string& base58str)
        {
            try {
                ...
                FC_ASSERT(base58str.size() > prefix_len, "Address must be longer than DFD_ADDRESS_PREFIX");
                FC_ASSERT(base58str.substr(0, prefix_len) == prefix, "DFD_ADDRESS_PREFIX must be in front of the address");
...
namespace fc
{
    void to_variant(const dfdcore::consensus::ExtendedAddress& ext_addr, fc::variant& vo)
    {
    ...
    {
        vo = dfdcore::consensus::ExtendedAddress(var.as_string());
    }
}
