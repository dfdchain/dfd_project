...
namespace dfdcore {
	namespace consensus {
		...
		Address::Address(const std::string& base58str, const AddressType& address_type)
		{
			...
			if (!Address::is_valid(base58str, DFD_ADDRESS_PREFIX))
			{
		...
		Address::Address(const WithdrawCondition& condition, const AddressType& address_type)
		{
			...
			if (address_type == AddressType::dfd_address)
				addr.data[0] = 30;
		...
		*/
		bool Address::is_valid(const std::string& base58str, const std::string& prefix)
		{
			...
			if (base58str.size() <= prefix_len)
				return false;
			if (base58str.substr(0, prefix_len) != DFD_ADDRESS_PREFIX && base58str.substr(0, prefix_len) != CONTRACT_ADDRESS_PREFIX && base58str.substr(0, prefix_len) != SCRIPT_ID_PREFIX &&base58str.substr(0, prefix_len) != MULTI_ADDRESS_PREFIX)
				return false;
		...
		{
			// dfd address
			if (base58str[0] == DFD_ADDRESS_PREFIX[0])
				return AddressType::dfd_address;
			else if (base58str[0] == CONTRACT_ADDRESS_PREFIX[0])
				return AddressType::contract_address;
			else if (base58str[0] == SCRIPT_ID_PREFIX[0])
				return AddressType::script_id;
			else if (base58str[0] == MULTI_ADDRESS_PREFIX[0])
		...
		Address::Address(const dfdcore::consensus::PublicKeyType& pub, const AddressType& address_type) : Address(fc::ecc::public_key(pub.key_data), address_type)
		{
		}
...
namespace fc
{
	void to_variant(const dfdcore::consensus::Address& var, variant& vo)
	{
		vo = std::string(var);
	}
	void from_variant(const variant& var, dfdcore::consensus::Address& vo)
	{
		vo = dfdcore::consensus::Address(var.as_string());
	}
}
