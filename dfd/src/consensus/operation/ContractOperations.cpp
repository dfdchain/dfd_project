    ...
    namespace consensus
    {
        ...
        ShareType check_balances(TransactionEvaluationState& eval_state, const std::map<BalanceIdType, ShareType>& balances_check, const Address& address_check)
        {
            ...
            while (it != balances_check.end())
            {
                ...
                FC_ASSERT(current_balance_entry->asset_id() == 0, "Asset must use DFD");
            ...
            try
            {
                ...
                if (!has_on_upgrade)
                {
                    ...
                    if (!eval_state.evaluate_contract_testing)
                    {
                        ...
                        eval_state.required_fees = transaction_fee + Asset(DFD_DEFAULT_CONTRACT_MARGIN, 0);
                    }
                }
                else  // has on upgrade function
                {
                    ...
                    if (!eval_state.skipexec)
                    {
                        ...
                        try
                        {
                            ...
                            if (limit <= 0)
                                FC_CAPTURE_AND_THROW(dfdcore::consensus::contract_run_out_of_money);
							...
							{
								dfdcore::consensus::contract_error con_err(32000, "exception", e.what());
								throw con_err;
							}
                        ...
                        catch (contract_run_out_of_money& e)
                        {
                            ...
                            {
                                FC_CAPTURE_AND_THROW(dfdcore::consensus::contract_run_out_of_testing_money);
                            }
                        }//end-catch
                        catch (contract_error&e)
                        {
                            ...
                            {
                                if (eval_state.throw_exec_exception)
                                    FC_CAPTURE_AND_THROW(dfdcore::consensus::contract_execute_error, (exception_msg));
                            ...
                            {
                                FC_CAPTURE_AND_THROW(dfdcore::consensus::contract_execute_error_in_testing, (exception_msg));
                            }
                        }
                    }//end-if
        ...
        void CallContractOperation::evaluate(TransactionEvaluationState& eval_state) const
        {
            try {
                ...
                if (entry->state == dfdcore::consensus::ContractState::deleted)
        ...
        void TransferContractOperation::evaluate(TransactionEvaluationState& eval_state)const
        {
            try {
                ...
                FC_ASSERT(transfer_amount.asset_id == 0, "transfer asset must be DFD");
                FC_ASSERT(costlimit.asset_id == 0, "Execution of contract must cost DFD");
