...
namespace dfdcore {
    ...
    namespace consensus {
        ...
        namespace detail
        {
            void ChainDatabaseImpl::revalidate_pending()
            {
                ...
                while (itr.valid())
                {
                    ...
                    try
                    {
                        ...
                        count++;
                        if (count > DFD_BLOCKCHAIN_REVALIDATE_MAX_TRX_COUNT)
                            break;
            ...
            bool ChainDatabaseImpl::replay_required(const fc::path& data_dir)
            {
                try {
                    ...
                    return !entry.valid() || entry->value.as_uint64() != DFD_BLOCKCHAIN_DATABASE_VERSION;
                } FC_CAPTURE_AND_RETHROW((data_dir))
            }
            ...
            void ChainDatabaseImpl::open_database(const fc::path& data_dir)
            {
                try {
                    ...
                    _dfd_input_balance_entry.open(data_dir / "index/_dfd_input_balance_entry");
					_dfd_full_entry.open(data_dir / "index/_dfd_full_entry");
            ...
            void ChainDatabaseImpl::clear_invalidation_of_future_blocks()
            {
                for (auto block_id_itr = _revalidatable_future_blocks_db.begin(); block_id_itr.valid();)
                {
                    ...
                    fc::time_point_sec now = dfdcore::consensus::now();
            ...
            DigestType ChainDatabaseImpl::initialize_genesis(const optional<path>& genesis_file, const bool statistics_enabled)
            {
                try {
                    ...
                    // Check genesis state
                    FC_ASSERT(config.delegates.size() >= DFD_BLOCKCHAIN_NUM_DELEGATES,
                        "genesis.json does not contain enough initial delegates!",
                        ("required", DFD_BLOCKCHAIN_NUM_DELEGATES)("provided", config.delegates.size()));
                    ...
                    {
                        //static const vector<string> dfd_prefixes{ "DFD" };
                        try
                        {
                        ...
                        {
                            //for( const string& prefix : dfd_prefixes )
                            //{
                            // if( raw_address.find( prefix ) == 0 )
                            //  {
                            //  return address( DFD_ADDRESS_PREFIX + raw_address.substr( prefix.size() ) );
                            // }
                            //}
                        }
                    ...
                    base_asset.symbol = DFD_BLOCKCHAIN_SYMBOL;
                    base_asset.name = DFD_BLOCKCHAIN_NAME;
                    base_asset.description = DFD_BLOCKCHAIN_DESCRIPTION;
                    base_asset.public_data = variant("");
                    base_asset.issuer_account_id = 0;
                    base_asset.precision = DFD_BLOCKCHAIN_PRECISION;
                    base_asset.registration_date = timestamp;
                    base_asset.last_update = timestamp;
                    base_asset.current_share_supply = total_base_supply;
                    base_asset.maximum_share_supply = DFD_BLOCKCHAIN_MAX_SHARES;
                    ...
                    for (const GenesisAsset& asset : config.market_assets)
                    {
                        ...
                        rec.maximum_share_supply = DFD_BLOCKCHAIN_MAX_SHARES;
                ...
                const FullBlock& block_data)
            {
                try {
                    ...
                    if (self->get_statistics_enabled())
                    {
                        ...
                        entry.latency = dfdcore::consensus::now() - block_data.timestamp;
                ...
                const PendingChainStatePtr& pending_state)
            {
                try {
                    ...
                    for (const auto& _trx : block_data.user_transactions)
                    {
                        ...
                        FC_ASSERT(trx.data_size() <= DFD_BLOCKCHAIN_MAX_TRX_SIZE, "trx size is out of the max trx size range");
                ...
                oBlockEntry& entry)const
            {
                try {
					...
					const ShareType max_new_shares = self->get_max_delegate_pay_issued_per_block(block_height);
                    ...
                    static const uint32_t blocks_per_two_weeks = 14 * DFD_BLOCKCHAIN_BLOCKS_PER_DAY;
                ...
                const PendingChainStatePtr& pending_state)
            {
                try {
                    ...
                    if (block_num > DFD_BLOCKCHAIN_MAX_UNDO_HISTORY)
                    {
                        const uint32_t old_block_num = block_num - DFD_BLOCKCHAIN_MAX_UNDO_HISTORY;
            ...
            void ChainDatabaseImpl::verify_header(const DigestBlock& block_digest, const PublicKeyType& block_signee)const
            {
                try {
                    ...
                    if (block_digest.timestamp.sec_since_epoch() % DFD_BLOCKCHAIN_BLOCK_INTERVAL_SEC != 0)
                    ...
                    if (block_digest.timestamp > (now + DFD_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 2))
            ...
            /**
             *  A block should be produced every DFD_BLOCKCHAIN_BLOCK_INTERVAL_SEC. If we do not have a
                ...
                const PendingChainStatePtr& pending_state)const
            {
                try {
                    ...
                    if (head_block.block_num > 0) block_timestamp = head_block.timestamp + DFD_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                    ...
                    for (; block_timestamp < block_header.timestamp;
                        block_timestamp += DFD_BLOCKCHAIN_BLOCK_INTERVAL_SEC,
                    ...
                    if (required_confirmations > DFD_BLOCKCHAIN_NUM_DELEGATES * 3)
                        required_confirmations = 3 * DFD_BLOCKCHAIN_NUM_DELEGATES;
            ...
            {
                try {
                    if (block_num % DFD_BLOCKCHAIN_NUM_DELEGATES != 0)
                        return;
            ...
            void ChainDatabaseImpl::extend_chain(const FullBlock& block_data)
            {
                try {
                    ...
                    try
                    {
                        ...
                        pay_delegate(block_id, block_data.block_num, block_signee, pending_state, block_entry);
                        ...
                        if (dfdcore::client::g_client->get_wallet() != nullptr&&dfdcore::client::g_client->get_wallet()->is_open())
                        {
                            if (!dfdcore::client::g_client->get_wallet()->get_my_delegates(dfdcore::wallet::enabled_delegate_status).empty())
                            {
                                auto fee = dfdcore::client::g_client->get_delegate_config().transaction_min_fee;
                                _relay_fee = fee > DFD_BLOCKCHAIN_DEFAULT_RELAY_FEE ? fee : DFD_BLOCKCHAIN_DEFAULT_RELAY_FEE;
                            }
                            else
                            {
                                auto fee = dfdcore::client::g_client->get_wallet()->get_transaction_fee();
                                _relay_fee = fee.amount > DFD_BLOCKCHAIN_DEFAULT_RELAY_FEE ? fee.amount : DFD_BLOCKCHAIN_DEFAULT_RELAY_FEE;
                                //_relay_fee = DFD_BLOCKCHAIN_DEFAULT_RELAY_FEE;
                            }
                        }
                    }
                    catch (const fc::exception& e)
                    ...
                    if ((now() - block_data.timestamp).to_seconds() < DFD_BLOCKCHAIN_BLOCK_INTERVAL_SEC)
            ...
            void ChainDatabaseImpl::pop_block()
            {
                try {
                    ...
                    dfdcore::consensus::PendingChainStatePtr undo_state_ptr = std::make_shared<dfdcore::consensus::PendingChainState>(undo_state);
        ...
        {
            return get_delegates_by_vote(0, DFD_BLOCKCHAIN_NUM_DELEGATES);
        }
            ...
            try {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        my->open_database(data_dir);
                        store_property_entry(PropertyIdType::database_version, variant(DFD_BLOCKCHAIN_DATABASE_VERSION));
                        ...
                        const auto start_time = dfdcore::consensus::now();
                        ...
                        const auto insert_block = [&](const FullBlock& block)
                        {
                            if (blocks_indexed % 200 == 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    progress = float(blocks_indexed * DFD_BLOCKCHAIN_BLOCK_INTERVAL_SEC) / seconds;
                                }
                        ...
                        try
                        {
                            ...
                            else
                            {
                                ...
                                if (last_known_block_num > DFD_BLOCKCHAIN_MAX_UNDO_HISTORY)
                                    my->_min_undo_block = last_known_block_num - DFD_BLOCKCHAIN_MAX_UNDO_HISTORY;
                        ...
                        catch (dfdcore::consensus::store_and_index_a_seen_block&  e)
                        {
                        ...
                        std::cout << "\rSuccessfully replayed " << blocks_indexed << " blocks in "
                            << (dfdcore::consensus::now() - start_time).to_seconds() << " seconds.                          "
        ...
        void ChainDatabase::close()
        {
            try {
                ...
                my->_dfd_input_balance_entry.close();
                my->_dfd_full_entry.close();
        ...
        {
            try {
                auto slot_number = dfdcore::consensus::get_slot_number(timestamp);
                auto delegate_pos = slot_number % DFD_BLOCKCHAIN_NUM_DELEGATES;
        ...
        {
            try {
                auto next_block_time = dfdcore::consensus::get_slot_start_time(dfdcore::consensus::now());
                if (next_block_time <= now()) next_block_time += DFD_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                auto last_block_time = next_block_time + (DFD_BLOCKCHAIN_NUM_DELEGATES * DFD_BLOCKCHAIN_BLOCK_INTERVAL_SEC);

                auto active_delegates = get_active_delegates();
                for (; next_block_time < last_block_time; next_block_time += DFD_BLOCKCHAIN_BLOCK_INTERVAL_SEC)
                {
                    auto slot_number = dfdcore::consensus::get_slot_number(next_block_time);
                    auto delegate_pos = slot_number % DFD_BLOCKCHAIN_NUM_DELEGATES;
            ...
            const ShareType required_fees, bool contract_vm_exec, bool skip_signature_check, bool throw_exec_exception)
        {
            try {
                ...
                catch (dfdcore::consensus::ignore_check_required_fee_state& e)
                {
        ...
        BlockForkData ChainDatabase::push_block(const FullBlock& block_data)
        {
            try {
                ...
                if (head_block_num > DFD_BLOCKCHAIN_MAX_UNDO_HISTORY &&
                    block_data.block_num <= (head_block_num - DFD_BLOCKCHAIN_MAX_UNDO_HISTORY))
                {
                    elog("block ${new_block_hash} (number ${new_block_num}) is on a fork older than "
                        ...
                        ("head_block_num", head_block_num)("undo_history", DFD_BLOCKCHAIN_MAX_UNDO_HISTORY));
                ...
                */
                ...
                else
                {
                    ...
                    dfdcore::consensus::ntp_time();
                    // 		 dfdcore::consensusupdate_ntp_time();
        ...
        fc::time_point_sec ChainDatabase::now()const
        {
            ...
            {
                auto slot_number = dfdcore::consensus::get_slot_number(dfdcore::consensus::now());
                return dfdcore::consensus::get_slot_start_time(slot_number - 1);
            }
        ...
        */
        double ChainDatabase::get_average_delegate_participation()const
        {
            try {
                ...
                const auto now = dfdcore::consensus::now();
                if (head_num < 1)
                {
                    return 0;
                }
                else if (head_num <= DFD_BLOCKCHAIN_NUM_DELEGATES)
                {
                    // what percent of the maximum total blocks that could have been produced
                    // have been produced.
                    const auto expected_blocks = (now - get_block_header(1).timestamp).to_seconds() / DFD_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                ...
                else
                {
                    ...
                    const auto starting_time = get_block_header(head_num - DFD_BLOCKCHAIN_NUM_DELEGATES).timestamp;
                    const auto expected_production = (now - starting_time).to_seconds() / DFD_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                    return 100 * double(DFD_BLOCKCHAIN_NUM_DELEGATES) / expected_production;
                }
            } FC_RETHROW_EXCEPTIONS(warn, "")
        }
        ...
        TransactionEvaluationStatePtr ChainDatabase::store_pending_transaction(const SignedTransaction& trx, bool override_limits, bool contract_vm_exec, bool cache)
        {
            try {
                ...
                if (!override_limits)
                {
                    if (my->_pending_fee_index.size() > DFD_BLOCKCHAIN_MAX_PENDING_QUEUE_SIZE)
                    {
                        auto overage = my->_pending_fee_index.size() - DFD_BLOCKCHAIN_MAX_PENDING_QUEUE_SIZE;
                        ...
                        try
                        {
                            ...
                            auto pending_trx_state = origin_trx_state;
                            {
                                ...
                                if (trx_eval_state->p_result_trx.operations.size() > 0)
                                {
                                    ...
                                    if (iMessageLength > DFD_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE)
                                    {
                                        imessage_fee = config.transaction_imessage_min_fee_coe * (iMessageLength - DFD_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE);
                                    }
        ...
        string ChainDatabase::export_fork_graph(uint32_t start_block, uint32_t end_block, const fc::path& filename)const
        {
            ...
            for (auto block_itr = my->_block_id_to_full_block.begin(); block_itr.valid(); ++block_itr)
            {
                ...
                {
                    unsigned rank = (unsigned)((block.timestamp - start_time).to_seconds() / DFD_BLOCKCHAIN_BLOCK_INTERVAL_SEC);
        ...
        bool ChainDatabase::store_account_entries_for_simulator(const vector<dfdcore::wallet::WalletAccountEntry>& vec_account_entry)
        {
        ...
        vector<DfdTrxidBalance> ChainDatabase::fetch_dfd_input_balance(const uint32_t & block_num)
        {
            try{
                vector < DfdTrxidBalance > results;
                for (auto iter = my->_dfd_input_balance_entry.unordered_begin();
                    iter != my->_dfd_input_balance_entry.unordered_end(); ++iter)
                {
                    DfdTrxidBalance dfdTemp;
					dfdTemp.block_num = block_num + 1;
					set<DfdTrxidBalance>::iterator block_iter = iter->second.lower_bound(dfdTemp);
        ...
        vector<DfdTrxidBalance> ChainDatabase::fetch_dfd_full_entry(const uint32_t& block_num, const uint32_t & last_scan_block_num)
        {
            try{
                vector < DfdTrxidBalance > results;
                ...
                for (auto iter = my->_dfd_full_entry.unordered_begin();
                    iter != my->_dfd_full_entry.unordered_end(); ++iter)
                {
                    auto block_iter = iter->second.dfd_block_sort.lower_bound(block_num + 1);
                    for (; block_iter != iter->second.dfd_block_sort.end(); block_iter++)
                    {
        ...
        void ChainDatabase::transaction_insert_to_dfd_full_entry(const string & dfd_account, const DfdTrxidBalance & dfd_balance_record)
        {
            auto iter_entry = my->_dfd_full_entry.unordered_find(dfd_account);
			DfdBalanceEntry dfdentrys;
            if (iter_entry != my->_dfd_full_entry.unordered_end())
            {
				dfdentrys = iter_entry->second;
				if (dfdentrys.dfd_trxid_sort.count(dfd_balance_record.trx_id) != 0)
                {
                    return;
                }
            }
			dfdentrys.dfd_trxid_sort.insert(dfd_balance_record.trx_id);
			dfdentrys.dfd_block_sort.insert(std::make_pair(dfd_balance_record.block_num, dfd_balance_record));
			my->_dfd_full_entry.store(dfd_account, dfdentrys);
        }
        void ChainDatabase::transaction_erase_from_dfd_full_entry(const string & dfd_account, const DfdTrxidBalance & dfd_balance_record)
        {
            auto iter_entry = my->_dfd_full_entry.unordered_find(dfd_account);
            if (iter_entry != my->_dfd_full_entry.unordered_end())
            {
                DfdBalanceEntry dfdentrys = iter_entry->second;
				dfdentrys.dfd_trxid_sort.erase(dfd_balance_record.trx_id);
				auto iter = dfdentrys.dfd_block_sort.lower_bound(dfd_balance_record.block_num);
				for (; iter != dfdentrys.dfd_block_sort.end();)
                {
                    if (iter->second.block_num != dfd_balance_record.block_num)
                    {
                        break;
                    }
                    if (iter->second.trx_id == dfd_balance_record.trx_id)
                    {
						iter = dfdentrys.dfd_block_sort.erase(iter);
                    }
                    else
                    {
                        ++iter;
				...
				if (!(dfdentrys.dfd_block_sort.empty()))
                {
					my->_dfd_full_entry.store(dfd_account, dfdentrys);
                }
                else
                {
                    my->_dfd_full_entry.remove(dfd_account);
                }
            }
        }
        void ChainDatabase::transaction_insert_to_dfd_balance(const string & dfd_account, const DfdTrxidBalance & dfd_balance_entry)
        {
            auto iter_ids = my->_dfd_input_balance_entry.unordered_find(dfd_account);
            set<DfdTrxidBalance> ids;
            if (iter_ids != my->_dfd_input_balance_entry.unordered_end())
            {
                ids = iter_ids->second;
            }
            ids.insert(dfd_balance_entry);
            my->_dfd_input_balance_entry.store(dfd_account, ids);
        }
        void ChainDatabase::transaction_erase_from_dfd_balance(const string & dfd_account, const DfdTrxidBalance & dfd_balance_entry)
        {
            auto iter_ids = my->_dfd_input_balance_entry.unordered_find(dfd_account);
            if (iter_ids != my->_dfd_input_balance_entry.unordered_end())
            {
                auto ids = iter_ids->second;
                ids.erase(dfd_balance_entry);
                if (!(ids.empty()))
                {
                    my->_dfd_input_balance_entry.store(dfd_account, ids);
                }
                ...
                {
                    my->_dfd_input_balance_entry.remove(dfd_account);
                }
            }
        }
        ...
        void ChainDatabase::dump_state(const fc::path& path)const
        {
            try {
                ...
                */
                ...
                ulog("Dumped ${p}", ("p", next_path));
                next_path = dir / "_dfd_transaction_balance_db.json";
                my->_dfd_input_balance_entry.export_to_json(next_path);
				ulog("Dumped ${p}", ("p", next_path));
        ...
        oResultTIdEntry dfdcore::consensus::ChainDatabase::contract_lookup_resultid_by_reqestid(const TransactionIdType & id) const
        {
        ...
        void dfdcore::consensus::ChainDatabase::contract_store_resultid_by_reqestid(const TransactionIdType & req, const ResultTIdEntry & res)
        {
            my->_request_to_result_iddb.store(req, res);
        }

        void dfdcore::consensus::ChainDatabase::contract_erase_resultid_by_reqestid(const TransactionIdType & req)
        {
            my->_request_to_result_iddb.remove(req);
        }

        oRequestIdEntry dfdcore::consensus::ChainDatabase::contract_lookup_requestid_by_resultid(const TransactionIdType &id) const
        {
        ...
        void dfdcore::consensus::ChainDatabase::contract_store_requestid_by_resultid(const TransactionIdType & res, const RequestIdEntry & req)
        {
            my->_result_to_request_iddb.store(res, req);
        }

        void dfdcore::consensus::ChainDatabase::contract_erase_requestid_by_resultid(const TransactionIdType & result)
        {
            my->_result_to_request_iddb.remove(result);
        }

        oContractinTrxEntry dfdcore::consensus::ChainDatabase::contract_lookup_contractid_by_trxid(const TransactionIdType &id) const
        {
        ...
        oContractTrxEntry dfdcore::consensus::ChainDatabase::contract_lookup_trxid_by_contract_id(const ContractIdType &id) const
        {
        ...
        void dfdcore::consensus::ChainDatabase::contract_store_contractid_by_trxid(const TransactionIdType & tid, const ContractinTrxEntry & cid)
        {
            my->_trx_to_contract_iddb.store(tid, cid);
        }

        void dfdcore::consensus::ChainDatabase::contract_store_trxid_by_contractid(const ContractIdType & cid, const ContractTrxEntry & tid)
        {
            my->_contract_to_trx_iddb.store(cid, tid);
        }

        void dfdcore::consensus::ChainDatabase::contract_erase_trxid_by_contract_id(const ContractIdType &id)
        {
            my->_contract_to_trx_iddb.remove(id);
        }

        void dfdcore::consensus::ChainDatabase::contract_erase_contractid_by_trxid(const TransactionIdType & tid)
        {
