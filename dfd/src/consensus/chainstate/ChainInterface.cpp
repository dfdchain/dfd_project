...
namespace dfdcore {
    namespace consensus {
        ...
        {
            try {
                if (name.size() < DFD_BLOCKCHAIN_MIN_NAME_SIZE) return false;
                if (name.size() > DFD_BLOCKCHAIN_MAX_NAME_SIZE) return false;
        ...
        {
            try {
                FC_ASSERT(symbol != "DFD", "Symbol must not be DFD");

                if (symbol.size() < DFD_BLOCKCHAIN_MIN_SYMBOL_SIZE)
                ...
                if (dots == 0)
                {
                    if (position > DFD_BLOCKCHAIN_MAX_SDFD_SYMBOL_SIZE)
                        FC_ASSERT(false, "Symbol name too large");
                }
                else //dots == 1 means hierarchial asset name
                {
                    if (position - dot_position - 1 > DFD_BLOCKCHAIN_MAX_SDFD_SYMBOL_SIZE)
                        FC_ASSERT(false, "Sub-symbol name too large");
                    if (symbol.size() > DFD_BLOCKCHAIN_MAX_SYMBOL_SIZE)
                        FC_ASSERT(false, "Symbol name too large");
                }

                if (symbol.find("DFD") == 0)
                    FC_ASSERT(false, "Symbol names cannot be prefixed with DFD");
        ...
        {
			return ShareType(0);
            ShareType pay_per_block = DFD_MAX_DELEGATE_PAY_PER_BLOCK;
			const auto share_entry = get_asset_entry(DFD_BLOCKCHAIN_SYMBOL);
		...
		ShareType ChainInterface::get_max_delegate_pay_issued_per_block(uint32_t block_height)const
		{
			if (block_height < HARD_FORK_REWARD_HEIGHT) {
				return ShareType(0);
			}
			
			uint32_t period = (block_height - HARD_FORK_REWARD_HEIGHT) / REWARD_HALF_BLOCK_COUNT;
			
			return DFD_MAX_DELEGATE_PAY_PER_BLOCK >> period;
		}

        ShareType ChainInterface::get_delegate_registration_fee(uint8_t pay_rate)const
        {
            if (pay_rate == 0) return 0;
			return ShareType((int64_t)1000000 * DFD_BLOCKCHAIN_PRECISION);
            ...
            const ShareType max_pay_per_two_weeks = max_total_pay_per_two_weeks / DFD_BLOCKCHAIN_NUM_DELEGATES;
        ...
        {
            if (imessage.size() < DFD_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE)
            {
                return 0;
            }
            ImessageIdType fee_coe = DFD_BLOCKCHAIN_MIN_MESSAGE_FEE_COE;
            ShareType fee = 0;
            fee = fee_coe * (imessage.size() - DFD_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE);
            return fee;
        }
        ShareType ChainInterface::get_asset_registration_fee(uint8_t symbol_length)const
        {
            // TODO: Add #define's for these fixed prices
            static const ShareType long_symbol_price = 500 * DFD_BLOCKCHAIN_PRECISION; //
        ...
        double ChainInterface::to_pretty_price_double(const Price& price_to_pretty_print)const
        {
            ...
            return fc::variant(string(price_to_pretty_print.ratio * obase_asset->precision / oquote_asset->precision)).as_double() / (DFD_BLOCKCHAIN_MAX_SHARES * 1000);
        }
        ...
        uint64_t ChainInterface::get_required_confirmations()const
        {
            try {
                ...
                return DFD_BLOCKCHAIN_NUM_DELEGATES * 3;
            } FC_CAPTURE_AND_RETHROW()
        }
        ...
        dfdcore::consensus::Asset ChainInterface::get_amount(ShareType limit, AssetIdType asset_id)
        {
        ...
        {
            Asset fee(DFD_DEFAULT_TRANSACTION_FEE, 0);
            return fee;
        }
        ...
        {
            Asset fee(DFD_DEFAULT_CONTRACT_MARGIN, 0);
            return fee;
        }
        ...
        {
            return DFD_DEFAULT_TRANSACTION_EXPIRATION_SEC;
        }
        */
            ...
            const Address& to_account_address, SignedTransaction& trx)
        {
            try {
				...
				{
					FC_CAPTURE_AND_THROW(invalid_asset_symbol,("Only DFD Allowed"));
				}
