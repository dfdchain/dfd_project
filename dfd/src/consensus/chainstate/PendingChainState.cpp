...
namespace dfdcore {
    namespace consensus {
        ...
        {
            const ChainInterfacePtr prev_state = _prev_state.lock();
            if (!prev_state) return get_slot_start_time(dfdcore::consensus::now());
        ...
        dfdcore::consensus::BlockIdType PendingChainState::get_block_id(uint32_t block_num) const
        {
            const ChainInterfacePtr prev_state = _prev_state.lock();
        ...
        dfdcore::consensus::SignedBlockHeader PendingChainState::get_block_header(const BlockIdType& id) const
        {
            const ChainInterfacePtr prev_state = _prev_state.lock();
		...
		dfdcore::consensus::oContractTemplateEntry PendingChainState::contracttemplate_lookup_by_hash(const std::string& hash)const
		{
