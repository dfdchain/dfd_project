/**
 * lua module injector header in dfdcore
 */
    ...
    namespace lua {
        namespace api {
            ...
            */
            ...
            /**
            * when exception happened, use this api to tell dfdcore
            ...
            */
            void UvmChainApi::throw_exception(lua_State *L, int code, const char *error_format, ...)
            {
                ...
                // 如果上次的exception code为DFDCORE_API_LVM_LIMIT_OVER_ERROR, 不能被其他异常覆盖
            ...
            */
            ...
            {
                return 0; // FIXME: need fill by dfdcore api
            }

            int UvmChainApi::get_stored_contract_info(lua_State *L, const char *name, std::shared_ptr<UvmContractInfo> contract_info_ret)
            {
                dfdcore::consensus::TransactionEvaluationState* pevaluate_state = (dfdcore::consensus::TransactionEvaluationState*)uvm::lua::lib::get_lua_state_value(L, "evaluate_state").pointer_value;
                // name = lua::lib::unwrap_any_contract_name(name).c_str();
                dfdcore::consensus::oContractEntry entry = pevaluate_state->_current_state->get_contract_entry(name);
                if (!entry.valid())
                    return 0;
            ...
            {
                dfdcore::consensus::TransactionEvaluationState* pevaluate_state = (dfdcore::consensus::TransactionEvaluationState*)uvm::lua::lib::get_lua_state_value(L, "evaluate_state").pointer_value;
                dfdcore::consensus::oContractEntry entry = pevaluate_state->_current_state->get_contract_entry(dfdcore::consensus::Address(std::string(address), AddressType::contract_address));
            ...
            void UvmChainApi::get_contract_address_by_name(lua_State *L, const char *name, char *address, size_t *address_size)
            {
                ...
                */
                dfdcore::consensus::TransactionEvaluationState* eval_state_ptr =
                    (dfdcore::consensus::TransactionEvaluationState*)
                ...
                dfdcore::consensus::ChainInterface* cur_state = eval_state_ptr->_current_state;
            ...
            bool UvmChainApi::check_contract_exist_by_address(lua_State *L, const char *address)
            {
                ...
                oContractEntry entry = cur_state->get_contract_entry(dfdcore::consensus::Address(std::string(address), AddressType::contract_address));
            ...
            {
                /*
                char *filename = lutil_concat_str4("dfdcore_lua_modules", file_separator_str(), "dfdcore_contract_", name);
            ...
            /**
            * load contract lua byte stream from dfdcore api
            */
            ...
            /**
            * store contract lua module byte stream to dfdcore api
            */
            //need to delete
            /*
            ...
            */
            ...
            UvmStorageValue UvmChainApi::get_storage_value_from_uvm_by_address(lua_State *L, const char *contract_address, std::string name)
            {
                ...
                dfdcore::consensus::StorageDataType storage_data = iter->second;

                return dfdcore::consensus::StorageDataType::create_lua_storage_from_storage_data(L, storage_data);
            }
            ...
            {
                // TODO
                printf("registered storage %s[%s] to dfdcore\n", contract_name, name);
                return true;
            }
                ...
                const char *asset_type, int64_t amount)
            {
                ...
                try
                {
                    if (!Address::is_valid(contract_address, CONTRACT_ADDRESS_PREFIX))
                        return -3;
                    if (!Address::is_valid(to_addr, DFD_ADDRESS_PREFIX))
                        return -4;

                    eval_state_ptr->transfer_asset_from_contract(amount, asset_type,
                        Address(string(contract_address), AddressType::contract_address), Address(to_addr, AddressType::dfd_address));
            ...
            int64_t UvmChainApi::get_contract_balance_amount(lua_State *L, const char *contract_address, const char* asset_symbol)
            {
                ...
                try{
                    dfdcore::consensus::TransactionEvaluationState* eval_state_ptr =
                        (dfdcore::consensus::TransactionEvaluationState*)
                        (uvm::lua::lib::get_lua_state_value(L, "evaluate_state").pointer_value);
                    dfdcore::consensus::ChainInterface* cur_state;
                    ...
                    {
                        FC_CAPTURE_AND_THROW(unknown_asset_id, ("Only DFD Allowed"));
                    }
			...
			bool UvmChainApi::is_valid_address(lua_State *L, const char *address_str)
			{
				...
				return Address::is_valid(addr, DFD_ADDRESS_PREFIX) || Address::is_valid(addr, CONTRACT_ADDRESS_PREFIX);
			}
			...
			{
				return DFD_BLOCKCHAIN_SYMBOL;
			}

			uint64_t UvmChainApi::get_system_asset_precision(lua_State *L)
			{
				return DFD_BLOCKCHAIN_PRECISION;
			}
