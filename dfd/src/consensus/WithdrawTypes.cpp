...
namespace dfdcore {
    namespace consensus {
            ...
            bool use_stealth_address)
        {
            ...
            if (memo_message.size() <= DFD_BLOCKCHAIN_MAX_MEMO_SIZE)
            {
    ...
    void from_variant(const variant& var, dfdcore::consensus::MemoData& vo)
    {
        try {
            ...
            if (obj.contains("from"))
                vo.from = obj["from"].as<dfdcore::consensus::PublicKeyType>();
            ...
            if (obj.contains("memo_flags"))
                vo.memo_flags = obj["memo_flags"].as<dfdcore::consensus::MemoFlagsEnum>();
