...
namespace dfdcore {
    namespace consensus {
        ...
        void TransactionEvaluationState::evaluate(const SignedTransaction& trx_arg)
        {
            ...
            try {
                ...
                try {
                    ...
                    if ((_current_state->now() + DFD_BLOCKCHAIN_BLOCK_INTERVAL_SEC + DFD_BLOCKCHAIN_MAX_TRANSACTION_EXPIRATION_SEC) < trx_arg.expiration)
                        FC_CAPTURE_AND_THROW(invalid_transaction_expiration, (trx_arg)(_current_state->now()));

                    if (origin_trx_basic_verify(trx) && (trx.data_size() > DFD_BLOCKCHAIN_MAX_TRX_SIZE))
                    {
					...
					if (num_of_signature > DFD_BLOCKCHAIN_MAX_SIGNAGTURE_NUM)
					{
						FC_CAPTURE_AND_THROW(too_much_signature, (DFD_BLOCKCHAIN_MAX_SIGNAGTURE_NUM)(num_of_signature));
					}
					...
					int signum_to_charge = num_of_signature - DFD_BLOCKCHAIN_FREESIGNATURE_LIMIT;
					if (signum_to_charge>=0)
					{
						required_fees.amount += signum_to_charge*DFD_BLOCKCHAIN_EXTRA_SIGNATURE_FEE;
					}
        ...
        SignedTransaction TransactionEvaluationState::simulator_evaluate(const SignedTransaction &trx_arg, bool& ignore_check_required_fee)
        {
            try
            {
                ...
                try
                {
                    ...
                    if ((_current_state->now() + DFD_BLOCKCHAIN_MAX_TRANSACTION_EXPIRATION_SEC) < trx_arg.expiration)
                        FC_CAPTURE_AND_THROW(invalid_transaction_expiration, (trx_arg)(_current_state->now()));
        ...
        bool TransactionEvaluationState::is_contract_op(const dfdcore::consensus::OperationTypeEnum& op_type)const
        {
            if (op_type >= dfdcore::consensus::OperationTypeEnum::contract_register_op_type
                && op_type <= dfdcore::consensus::OperationTypeEnum::transfer_contract_op_type)
                return true;
            return false;
        }
        ...
        bool TransactionEvaluationState::origin_trx_basic_verify(const SignedTransaction& trx)const
        {
            ...
            for (const auto& op : trx.operations)
            {
                if (op.type == dfdcore::consensus::OperationTypeEnum::transaction_op_type)
                    return false;
